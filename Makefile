...
install_inner:
	...
	if ! venv/bin/python --version; then rm -rf venv; source ./.bash_profile && python3 -m venv venv && source venv/bin/activate && python bin/get-pip.py && pip install --upgrade pip; fi
	source venv/bin/activate && pip install --no-cache-dir -r requirements.txt
