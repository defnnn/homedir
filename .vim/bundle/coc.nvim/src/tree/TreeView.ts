...
export default class BasicTreeView<T> implements TreeView<T> {
  ...
  private _onDidChangeSelection = new Emitter<
    TreeViewSelectionChangeEvent<T>
  >();
  private _onDidChangeVisibility = new Emitter<TreeViewVisibilityChangeEvent>();
  public onDidExpandElement: Event<TreeViewExpansionEvent<T>> =
    this._onDidExpandElement.event;
  public onDidCollapseElement: Event<TreeViewExpansionEvent<T>> =
    this._onDidCollapseElement.event;
  public onDidChangeSelection: Event<TreeViewSelectionChangeEvent<T>> =
    this._onDidChangeSelection.event;
  public onDidChangeVisibility: Event<TreeViewVisibilityChangeEvent> =
  ...
  constructor(private viewId: string, opts: TreeViewOptions<T>) {
    ...
    if (opts.enableFilter) {
      this.filter = new Filter(this.nvim, [
        this.keys.selectNext,
        this.keys.selectPrevious,
        this.keys.invoke,
      ]);
    }
    ...
    this.disposables.push(
      ...
      this._onDidCollapseElement,
      this._onDidExpandElement
    );
    if (this.filter) {
  ...
  private get startLnum(): number {
    ...
    return (
      this.lineState.messageCount + this.lineState.titleCount + filterCount
    );
  }
  ...
  private async doFilter(text: string): Promise<void> {
    ...
    try {
      ...
      for (let n of this.itemsToFilter) {
        ...
        let label = TreeItemLabel.is(item.label)
          ? item.label.label
          : item.label;
  ...
  private async onHover(): Promise<void> {
    ...
    let isMarkdown =
      MarkupContent.is(item.tooltip) &&
      item.tooltip.kind == MarkupKind.Markdown;
    let doc: Documentation = {
      filetype: isMarkdown ? "markdown" : "txt",
      content: MarkupContent.is(item.tooltip)
        ? item.tooltip.value
        : item.tooltip,
    };
  ...
  private async onClick(element: T): Promise<void> {
    let { nvim } = this;
    let [line, col] = (await nvim.eval(`[getline('.'),col('.')]`)) as [
      string,
      number
    ];
  ...
  private async invokeActions(element: T): Promise<void> {
    ...
    let obj = this.nodesMap.get(element);
    let actions = await Promise.resolve(
      this.provider.resolveActions(obj.item, element)
    );
  ...
  private async resolveItem(
    ...
    if (typeof this.provider.resolveTreeItem === "function") {
      let tokenSource = (this.resolveTokenSource =
        new CancellationTokenSource());
      let token = tokenSource.token;
      item = await Promise.resolve(
        this.provider.resolveTreeItem(item, element, token)
      );
      tokenSource.dispose();
  ...
  private getRenderedLine(
    treeItem: TreeItem,
    lnum: number,
    level: number
  ): { line: string; highlights: HighlightItem[] } {
    let { openedIcon, closedIcon } = this.config;
  ...
  private async appendTreeNode(
    ...
    if (treeItem.collapsibleState == TreeItemCollapsibleState.Expanded) {
      let l = level + 1;
      let children =
        (await Promise.resolve(this.provider.getChildren(element))) || [];
      for (let el of children) {
        let n = await this.appendTreeNode(
          el,
          l,
          lnum + takes,
          items,
  ...
  private updateUI(
    ...
    if (highlights.length) {
      ...
      nvim.call(
        ...
        [this.bufnr, highlightNamespace, highlights, start, highlightEnd],
        true
      );
    }
    ...
    try {
      if (this.message) {
        highlights.push({
          hlGroup: "MoreMsg",
          colStart: 0,
          colEnd: byteLength(this.message),
          lnum: 0,
        });
      ...
      if (this.title) {
        highlights.push({
          hlGroup: "CocTreeTitle",
          colStart: 0,
          colEnd: byteLength(this.title),
          lnum: lines.length,
        });
        if (this.description) {
          ...
          highlights.push({
            hlGroup: "Comment",
            colStart,
            colEnd: colStart + byteLength(this.description),
            lnum: lines.length,
          });
        }
        lines.push(
      ...
      this.nvim.errWriteLine(
        "[coc.nvim] Error on update head lines:" + e.message
      );
  ...
  public async show(splitCommand = "belowright 30vs"): Promise<void> {
    ...
    if (winid != -1) {
      ...
      nvim.command(
        `silent keepalt ${splitCommand} +setl\\ buftype=nofile CocTree${id}`,
        true
      );
    }
    nvim.command(
      "setl bufhidden=wipe nolist nonumber norelativenumber foldcolumn=0",
      true
    );
    nvim.command(
      `setl signcolumn=${this.canSelectMany ? "yes" : "no"}${
        this.winfixwidth ? " winfixwidth" : ""
      }`,
      true
    );
    nvim.command(
    ...
    nvim.command(
      `let w:cocViewId = "${this.viewId.replace(/"/g, '\\"')}"`,
      true
    );
  ...
  private registerKeymaps(): void {
    ...
    const regist = (
      mode: "n" | "v" | "s" | "x",
      key: string,
      fn: (element: T | undefined) => Promise<void>,
      notify = false
    ) => {
      this.disposables.push(
