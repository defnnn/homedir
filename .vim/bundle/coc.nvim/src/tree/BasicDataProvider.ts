 ...
 */
export default class BasicDataProvider<T extends TreeNode>
  implements TreeDataProvider<T>
{
  ...
  private iterate(
    ...
    level: number,
    fn: (node: T, parentNode: T | undefined, level: number) => void | boolean
  ): void | boolean {
  ...
  private updateNodes(
    ...
    if (sameNodes) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (curr.key) {
          findIndex = oldNodes.findIndex(
            (o, i) => !used.has(i) && o.key == curr.key
          );
        } else {
          findIndex = oldNodes.findIndex(
            (o, i) => !used.has(i) && o.label == curr.label
          );
        }
        if (findIndex === -1) {
          old[i] = curr;
  ...
  public async resolveTreeItem(
    ...
    if (typeof this.opts.resolveItem === "function") {
      let res = await Promise.resolve(
        this.opts.resolveItem(item, element, token)
      );
