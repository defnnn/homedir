...
export default class LanguageSource implements ISource {
  ...
  constructor(
    ...
    this.priority =
      typeof priority === "number" ? priority : completeConfig.priority;
  }
  ...
  public async onCompleteResolve(
    ...
    if (hasResolve) {
      ...
      try {
        let resolved = await Promise.resolve(
          this.provider.resolveCompletionItem(
            Object.assign({}, resolving),
            token
          )
        );
    ...
    if (typeof item.documentation === "undefined") {
      ...
      if (detail && !item.detailShown && detail != item.word) {
        ...
        if (detail.length) {
          let isText = /^[\w-\s.,\t\n]+$/.test(detail);
          docs.push({
            filetype: isText ? "txt" : this.filetype,
            content: detail,
          });
        }
      }
  ...
  public async onCompleteDone(
    vimItem: ExtendedCompleteItem,
    opt: CompleteOption
  ): Promise<void> {
    ...
    try {
      ...
      if (additionalTextEdits && item.textEdit) {
        let r = InsertReplaceEdit.is(item.textEdit)
      ...
      await this.applyAdditionalEdits(
        additionalTextEdits,
        opt.bufnr,
        isSnippet
      );
  ...
  private async applyTextEdit(
    ...
    let range = InsertReplaceEdit.is(textEdit)
      ? textEdit.replace
      : textEdit.range;
    ...
    if (isSnippet) {
      ...
      return await snippetManager.insertSnippet(
        newText,
        false,
        r,
        item.insertTextMode
    ...
    if (newLines.length == 1) {
      ...
      await window.moveTo(
        Position.create(linenr - 1, (start + newText).length)
      );
    } else {
  ...
  private async applyAdditionalEdits(
    ...
    if (changed)
      await window.moveTo(
        Position.create(
          pos.line + changed.line,
          pos.character + changed.character
        )
      );
  }
  ...
  private convertVimCompleteItem(
    ...
    let hasAdditionalEdit =
      item.additionalTextEdits && item.additionalTextEdits.length > 0;
    let isSnippet =
      item.insertTextFormat === InsertTextFormat.Snippet || hasAdditionalEdit;
...
export function getStartColumn(
  ...
  let range = InsertReplaceEdit.is(first.textEdit)
    ? first.textEdit.replace
    : first.textEdit.range;
  let { character } = range.start;
  for (let i = 1; i < Math.min(10, items.length); i++) {
    let o = items[i];
    if (!o.textEdit) return null;
    let r = InsertReplaceEdit.is(o.textEdit)
      ? o.textEdit.replace
      : o.textEdit.range;
...
export function getKindString(
  kind: CompletionItemKind,
  map: Map<CompletionItemKind, string>,
  defaultValue = ""
): string {
