...
export default class File extends Source {
    ...
    try {
      ...
      if (stat && stat.isDirectory()) {
        ...
        let items = await Promise.all(
          files.map((filename) => this.getFileItem(dir, filename))
        );
  ...
  public async doComplete(opt: CompleteOption): Promise<CompleteResult> {
    ...
    let startPart =
      opt.col == startcol ? "" : byteSlice(opt.line, opt.col, startcol);
    ...
    return {
      items: items.map((item) => {
        ...
        item.word =
          trimExt && ex === ext ? item.word.replace(ext, "") : item.word;
        return {
