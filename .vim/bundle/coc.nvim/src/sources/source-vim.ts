...
export default class VimSource extends Source {
  private async callOptinalFunc(fname: string, args: any[]): Promise<any> {
    ...
    try {
      ...
      window.showMessage(
        `Vim error from source ${this.name}: ${e.message}`,
        "error"
      );
      return null;
    }
  ...
  public async doComplete(
    ...
    let startcol: number | null = await this.callOptinalFunc("get_startcol", [
      opt,
    ]);
    ...
    let items: ExtendedCompleteItem[] = await this.nvim.callAsync(
