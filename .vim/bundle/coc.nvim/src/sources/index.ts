...
export class Sources {
  ...
  private loadCompleteConfig(): void {
    ...
    let labels = suggest.get<{ [key: string]: string }>(
    ...
    let map = new Map([
      ...
      [
        ...
        typeof labels["constructor"] == "function"
          ? "f"
          : labels["con" + "structor"],
      ],
    ...
    if (
      detailField == "preview" &&
      (!floatEnable || !workspace.floatSupported)
    ) {
      detailField = "menu";
    }
    this.completeConfig = Object.assign(this.completeConfig || {}, {
      ...
      invalidInsertCharacters: suggest.get<string[]>(
  ...
  private async createVimSourceExtension(
    nvim: Neovim,
    filepath: string
  ): Promise<void> {
    ...
    try {
      ...
      let packageJSON = {
        ...
        activationEvents: props.filetypes
          ? props.filetypes.map((f) => `onLanguage:${f}`)
          : ["*"],
        contributes: {
          configuration: {
            properties: {
      ...
      window.showMessage(
        `Error on create vim source ${name}: ${e.message}`,
        "error"
      );
    }
  }
  ...
  private async createVimSources(pluginPath: string): Promise<void> {
    ...
    if (stat && stat.isDirectory()) {
      ...
      await Promise.all(
        files.map((p) => this.createVimSourceExtension(this.nvim, p))
      );
    }
  }
  ...
  public shouldCommit(
    ...
    if (
      source &&
      source.sourceType == SourceType.Service &&
      typeof source.shouldCommit === "function"
    ) {
  ...
  public getNormalSources(filetype: string, uri: string): ISource[] {
    return this.sources.filter((source) => {
      ...
      if (
        !enable ||
        triggerOnly ||
        (filetypes && !filetypes.includes(filetype))
      ) {
        return false;
      }
  ...
  private checkTrigger(
    ...
    if (
      triggerPatterns &&
      triggerPatterns.findIndex((p) => p.test(pre)) !== -1
    ) {
      return true;
    }
    return false;
  ...
  public sourceStats(): SourceStat[] {
    ...
    for (let item of items) {
      res.push({
        ...
        type:
          item.sourceType == SourceType.Native
            ? "native"
            : item.sourceType == SourceType.Remote
            ? "remote"
            : "service",
  ...
  public createSource(config: SourceConfig): Disposable {
    ...
    let source = new Source(
      Object.assign({ sourceType: SourceType.Service } as any, config)
    );
