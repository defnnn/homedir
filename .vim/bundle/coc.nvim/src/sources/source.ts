...
export default class Source implements ISource {
  ...
  public get triggerOnly(): boolean {
    ...
    return (
      Array.isArray(this.triggerPatterns) && this.triggerPatterns.length != 0
    );
  }
  ...
  public getConfig<T>(key: string, defaultValue?: T): T | null {
    ...
    defaultValue = this.defaults.hasOwnProperty(key)
      ? this.defaults[key]
      : defaultValue;
  ...
  public async shouldComplete(opt: CompleteOption): Promise<boolean> {
    ...
    if (opt.synname && disableSyntaxes && disableSyntaxes.length) {
      ...
      if (
        disableSyntaxes.findIndex((s) => synname.includes(s.toLowerCase())) !==
        -1
      ) {
        return false;
      }
