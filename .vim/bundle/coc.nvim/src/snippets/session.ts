...
export class SnippetSession {
  ...
  constructor(private nvim: Neovim, public readonly bufnr: number) {
    ...
    this.preferComplete = suggest.get(
  ...
  public async start(
    ...
    if (insertTextMode === InsertTextMode.asIs) {
      ...
      inserted = normalizeSnippetString(
        snippetString,
        currentIndent,
        formatOptions
      );
  ...
  public async synchronizeUpdatedPlaceholders(
    ...
    if (
      ...
      changedLine.slice(currRange.start.character, currRange.end.character) ==
  ...
  public async selectPlaceholder(
    placeholder: CocSnippetPlaceholder,
    triggerAutocmd = true
  ): Promise<void> {
    ...
    const col =
      byteLength(document.getline(start.line).slice(0, start.character)) + 1;
    this._currId = placeholder.id;
    if (placeholder.choice) {
      await nvim.call("coc#snippet#show_choices", [
        start.line + 1,
        col,
        len,
        placeholder.choice,
  ...
  private async select(
    ...
    let [ve, selection, pumvisible, mode] = (await nvim.eval(
      "[&virtualedit, &selection, pumvisible(), mode()]"
    ...
    nvim.call(
      "cursor",
      [start.line + 1, col + (move_cmd == "a" ? 0 : 1)],
      true
    );
    if (move_cmd) {
...
export function normalizeSnippetString(
  ...
  lines = lines.map((line, idx) => {
    ...
    return (
      (idx == 0 || line.length == 0 ? "" : indent) +
      pre +
      line.slice(space.length)
    );
  });
  return lines.join("\n");
}
