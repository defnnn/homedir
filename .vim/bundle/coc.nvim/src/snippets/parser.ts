  ...
  public async resolve(resolver: VariableResolver): Promise<boolean> {
    ...
    if (value && value.includes("\n")) {
      ...
      let newLines = lines.map((s, i) =>
        i == 0 || s.length == 0 || !s.startsWith(minIndent)
          ? s
  ...
  public updateVariable(id: number, val: string): void {
    ...
    if (find) {
      ...
      for (let variable of variables) {
        let newText = variable.transform
          ? variable.transform.resolve(val)
          : val;
            ...
            if (marker instanceof Text) {
              ...
              for (let child of marker.children) {
                if (child instanceof Text) {
                  marker.replace(child, [
                    new Text(clone.transform.resolve(child.value)),
                  ]);
                  break;
                }
              }
