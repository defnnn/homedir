...
export class CocSnippet {
  ...
  public adjustTextEdit(edit: TextEdit): boolean {
    ...
    if (
      ...
      let idx = this._placeholders.findIndex(
        (o) => comparePosition(o.range.start, range.start) == 0
      );
  ...
  public get range(): Range {
    ...
    const end =
      pos.line == 0 ? position.character + pos.character : pos.character;
    return Range.create(
      position,
      Position.create(position.line + pos.line, end)
    );
  }
  ...
  public insertSnippet(
    ...
    let editEnd = Position.create(
      ...
      range.end.line == start.line
        ? range.end.character - start.character
        : range.end.character
    );
    let editRange = Range.create(editStart, editEnd);
    let first = this.tmSnippet.insertSnippet(
      snippet,
      placeholder.id,
      editRange
    );
