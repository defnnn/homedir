...
export default class CursorSession {
      ...
      if (isEnd) {
        if (textRange.currRange.start.character == range.start.character) {
          ...
          if (text.includes(textRange.text)) {
            ...
            if (offset > 0)
              ranges.forEach((r) =>
                r.replace(r.text.length - offset, r.text.length)
              );
          } else {
            this.cancel();
          }
  ...
  public addRange(range: Range, text: string): void {
    ...
    if (idx !== -1) {
      ...
      for (let r of ranges) {
        if (
          r.line > line ||
          (r.line == line && r.start > range.end.character)
        ) {
          break;
        }
  ...
  private async applySingleEdit(
    ...
    let { cursor } = events;
    if (
      textRange.preCount > 0 &&
      cursor.bufnr == doc.bufnr &&
      textRange.line + 1 == cursor.lnum
    ) {
      let changed =
        textRange.preCount *
        (newText.length - (range.end.character - range.start.character));
  ...
  private applyComposedEdit(original: string, edit: TextEdit): void {
    ...
    for (let i = 0; i < diffs.length; i++) {
      ...
      if (diff[0] == fastDiff.EQUAL) {
        ...
        let end = adjustPosition(
          range.start,
          doc.positionAt(offset + diff[1].length)
        );
        if (diffs[i + 1] && diffs[i + 1][0] == fastDiff.INSERT) {
          // change
          edits.push({
            range: Range.create(pos, end),
            newText: diffs[i + 1][1],
          });
          i = i + 1;
        } else {
    ...
    if (edits.length == ranges.length) {
      ...
      for (let i = 0; i < edits.length - 1; i = i + 2) {
        ...
        if (edit.newText.length == 0 && next.newText.length == 0) {
          ...
          let start =
            edit.range.start.character - textRange.currRange.start.character;
          textRange.replace(
            start,
