...
export class ServiceManager extends EventEmitter implements Disposable {
  ...
  private createCustomServices(): void {
    let lspConfig = workspace
      .getConfiguration()
      .get<{ key: LanguageServerConfig }>("languageserver", {} as any);
  ...
  private validServerConfig(
    ...
    if (config.transport != null && typeof config.transport !== "string") {
      errors.push(
        `"transport" field of languageserver ${key} should be string`
      );
    }
    if (
      config.transportPort != null &&
      typeof config.transportPort !== "number"
    ) {
      errors.push(
        `"transportPort" field of languageserver ${key} should be string`
      );
    }
    if (
      !Array.isArray(config.filetypes) ||
      !config.filetypes.every((s) => typeof s === "string")
    ) {
      errors.push(
        `"filetypes" field of languageserver ${key} should be array of string`
      );
    }
    if (
      config.additionalSchemes &&
      (!Array.isArray(config.additionalSchemes) ||
        config.additionalSchemes.some((s) => typeof s !== "string"))
    ) {
      errors.push(
        `"additionalSchemes" field of languageserver ${key} should be array of string`
      );
    }
    if (errors.length) {
  ...
  public async sendRequest(
    ...
    return await Promise.resolve(
      service.client.sendRequest(method, params, token)
    );
  }
  ...
  public registLanguageClient(
    ...
    let service: IServiceProvider = {
      ...
      selector:
        typeof name === "string"
          ? getDocumentSelector(config.filetypes, config.additionalSchemes)
          : name.clientOptions.documentSelector,
      ...
      start: (): Promise<void> => {
        ...
        if (!created) {
          if (typeof name == "string" && !client) {
            let config: LanguageServerConfig = workspace
              .getConfiguration()
              .get<{ key: LanguageServerConfig }>("languageserver", {} as any)[
              name
            ];
          ...
          client.onDidChangeState(
            (changeEvent) => {
              ...
              logger.info(
                `${client.name} state change: ${oldStr} => ${newStr}`
              );
            },
            null,
            disposables
...
export function documentSelectorToLanguageIds(
...
export function getLanguageServerOptions(
  id: string,
  name: string,
  config: LanguageServerConfig
  ...
  if (!filetypes) {
    window.showMessage(
      `Wrong configuration of LS "${name}", filetypes not found`,
      "error"
    );
    return null;
  }
  if (!command && !module && !port) {
    window.showMessage(
      `Wrong configuration of LS "${name}", no command or module specified.`,
      "error"
    );
    return null;
  }
  ...
  if (module) {
    ...
    if (!fs.existsSync(module)) {
      window.showMessage(
        `Module file "${module}" not found for LS "${name}"`,
        "error"
      );
      return null;
    }
  ...
  let clientOptions: LanguageClientOptions = {
    ...
    revealOutputChannelOn: getRevealOutputChannelOn(
...
export function getRevealOutputChannelOn(
  revealOn: string | undefined
): RevealOutputChannelOn {
  switch (revealOn) {
    case "info":
...
export function getDocumentSelector(
  ...
  filetypes.forEach((filetype) => {
    documentSelector.push(
      ...schemes.map((scheme) => ({ language: filetype, scheme }))
    );
  });
