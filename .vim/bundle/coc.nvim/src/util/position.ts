...
export function rangeInRange(r: Range, range: Range): boolean {
  return (
    positionInRange(r.start, range) === 0 && positionInRange(r.end, range) === 0
  );
}
...
export function adjustPosition(pos: Position, edit: TextEdit): Position {
  ...
  let pre =
    newLines.length == 1 && start.line != end.line ? start.character : 0;
  let removed =
    start.line == end.line && newLines.length == 1
...
export function editRange(range: Range, text: string, edit: TextEdit): string {
  ...
  let character =
    start.line == range.start.line
      ...
      : start.character;
  let startOffset = positionToOffset(
    lines,
    start.line - range.start.line,
    character
  );
  character =
      ...
      ? end.character - range.start.character
      : end.character;
  let endOffset = positionToOffset(
    lines,
    end.line - range.start.line,
    character
  );
  return `${text.slice(0, startOffset)}${edit.newText}${text.slice(
    endOffset,
    text.length
  )}`;
}

export function getChangedFromEdits(
  start: Position,
  edits: TextEdit[]
): Position | null {
  ...
  for (let edit of edits) {
    ...
    changed = {
      line: changed.line + d.line,
      character: changed.character + d.character,
    };
  }
