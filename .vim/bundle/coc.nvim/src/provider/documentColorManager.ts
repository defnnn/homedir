...
export default class DocumentColorManager extends Manager<DocumentColorProvider> {
  ...
  public async provideDocumentColors(
    ...
    if (!item) return null;
    let { provider } = item;
    let res: ColorInformation[] = await Promise.resolve(
      provider.provideDocumentColors(document, token)
    );
    return res;
  }
  ...
  public async provideColorPresentations(
    ...
    let res = await Promise.resolve(
      provider.provideColorPresentations(color, { document, range }, token)
    );
    return res;
  }
}
