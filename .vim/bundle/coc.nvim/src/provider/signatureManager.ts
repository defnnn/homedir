...
export default class SignatureManager extends Manager<SignatureHelpProvider> {
  public register(
    ...
    let characters = triggerCharacters.reduce(
      (p, c) => p.concat(c.length == 1 ? [c] : c.split(/\s*/g)),
      [] as string[]
    );
  ...
  public shouldTrigger(
    ...
    return (
      triggerCharacters && triggerCharacters.indexOf(triggerCharacter) != -1
    );
  }
  ...
  public async provideSignatureHelp(
    ...
    let res = await Promise.resolve(
      item.provider.provideSignatureHelp(document, position, token, context)
    );
