...
export default class Manager<T> {
  ...
  protected getProviders(document: TextDocument): ProviderItem<T>[] {
    ...
    items = items.filter(
      (item) => workspace.match(item.selector, document) > 0
    );
    return items.sort(
      (a, b) =>
        workspace.match(b.selector, document) -
        workspace.match(a.selector, document)
    );
  }

  protected toLocations(
    arr: (Definition | LocationLink[] | null)[]
  ): Location[] {
    let res: Location[] = [];
    for (let def of arr) {
      if (!def) continue;
      if (Location.is(def)) {
        ...
        for (let d of def) {
          if (Location.is(d)) {
            ...
            addLocation(
              res,
              Location.create(targetUri, targetSelectionRange || targetRange)
            );
          }
        }
      } else {
