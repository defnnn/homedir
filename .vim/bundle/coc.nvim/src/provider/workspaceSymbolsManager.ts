...
export default class WorkspaceSymbolManager {
  ...
  public async provideWorkspaceSymbols(
    ...
    await Promise.all(
      entries.map((o) => {
        let [id, p] = o;
        return Promise.resolve(p.provideWorkspaceSymbols(query, token)).then(
          (list) => {
            if (list)
              res.push(
                ...list.map((item) => Object.assign({ source: id }, item))
              );
          }
        );
      })
  ...
  public async resolveWorkspaceSymbol(
    ...
    return await Promise.resolve(
      provider.resolveWorkspaceSymbol(symbolInfo, token)
    );
  }
