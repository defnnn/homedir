...
export default class Plugin extends EventEmitter {
  ...
  constructor(public nvim: Neovim) {
    ...
    this.addAction("checkJsonExtension", () => {
      ...
      window.showMessage(
        `Run :CocInstall coc-json for json intellisense`,
        "more"
      );
    });
    ...
    this.addAction(
      "doKeymap",
      async (key: string, defaultReturn = "", pressed?: string) => {
        let keymap = workspace.keymaps.get(key);
        if (!keymap) {
          logger.error(`keymap for ${key} not found`);
          this.nvim.command(
            `silent! unmap <buffer> ${
              pressed.startsWith("{") && pressed.endsWith("}")
                ? `<${pressed.slice(1, -1)}>`
                : pressed
            }`,
            true
        ...
        if (repeat)
          await nvim.command(
            `silent! call repeat#set("\\<Plug>(coc-${key})", -1)`
          );
    ...
    this.addAction(
      "snippetCheck",
      async (checkExpand: boolean, checkJump: boolean) => {
    ...
    this.addAction("showInfo", async () => {
      ...
      channel.appendLine(
        "vim version: " +
          first +
          `${workspace.isVim ? " " + workspace.env.version : ""}`
      );
    ...
    this.addAction(
      ...
      (
        ...
        method: string,
        params: any,
        openCommand?: string | false
      ) => {
        return this.handler.locations.findLocations(
          id,
          method,
          params,
          openCommand
    ...
    this.addAction("search", (...args: string[]) =>
      this.handler.refactor.search(args)
    );
    this.addAction(
      "cursorsSelect",
      (bufnr: number, kind: string, mode: string) =>
        this.cursors.select(bufnr, kind, mode)
    );
    this.addAction("fillDiagnostics", (bufnr: number) =>
      diagnosticManager.setLocationlist(bufnr)
    );
    this.addAction("saveRefactor", (bufnr) =>
      this.handler.refactor.save(bufnr)
    );
    this.addAction("commandList", () => this.handler.commands.getCommandList());
    this.addAction(
      "selectSymbolRange",
      (inner: boolean, visualmode: string, supportedSymbols: string[]) =>
        this.handler.symbols.selectSymbolRange(
          inner,
          visualmode,
          supportedSymbols
        )
    ...
    this.addAction("sendRequest", (id: string, method: string, params?: any) =>
      services.sendRequest(id, method, params)
    );
    this.addAction(
      "sendNotification",
      (id: string, method: string, params?: any) =>
        services.sendNotification(id, method, params)
    );
    this.addAction("registNotification", (id: string, method: string) =>
      services.registNotification(id, method)
    );
    ...
    this.addAction("colorPresentation", () =>
      this.handler.colors.pickPresentation()
    );
    this.addAction("highlight", () =>
      this.handler.documentHighlighter.highlight()
    );
    this.addAction("fold", (kind?: string) => this.handler.fold.fold(kind));
    this.addAction("startCompletion", (option) =>
      completion.startCompletion(option)
    );
    ...
    this.addAction("diagnosticRefresh", (bufnr) =>
    ...
    this.addAction("diagnosticNext", (severity) =>
      diagnosticManager.jumpNext(severity)
    );
    this.addAction("diagnosticPrevious", (severity) =>
      diagnosticManager.jumpPrevious(severity)
    );
    ...
    this.addAction("symbolRanges", () =>
      this.handler.documentHighlighter.getSymbolsRanges()
    );
    this.addAction("selectionRanges", () =>
      this.handler.selectionRange.getSelectionRanges()
    );
    this.addAction("rangeSelect", (visualmode, forward) =>
      this.handler.selectionRange.selectRange(visualmode, forward)
    );
    this.addAction("rename", (newName) => this.handler.rename.rename(newName));
    this.addAction("getWorkspaceSymbols", (input) =>
      this.handler.symbols.getWorkspaceSymbols(input)
    );
    this.addAction("resolveWorkspaceSymbol", (symbolInfo) =>
      this.handler.symbols.resolveWorkspaceSymbol(symbolInfo)
    );
    this.addAction("formatSelected", (mode) =>
      this.handler.format.formatCurrentRange(mode)
    );
    ...
    this.addAction("codeAction", (mode, only) =>
      this.handler.codeActions.doCodeAction(mode, only)
    );
    this.addAction("organizeImport", () =>
      this.handler.codeActions.organizeImport()
    );
    this.addAction("fixAll", () =>
      this.handler.codeActions.doCodeAction(null, [CodeActionKind.SourceFixAll])
    );
    ...
    this.addAction("codeActions", async (mode, only) => {
      codeActions = await this.handler.codeActions.getCurrentCodeActions(
        mode,
        only
      );
    ...
    this.addAction("quickfixes", async (mode) => {
      codeActions = await this.handler.codeActions.getCurrentCodeActions(mode, [
    ...
    this.addAction("runCommand", (...args: any[]) =>
      this.handler.commands.runCommand(...args)
    );
    ...
    this.addAction("installExtensions", (...list: string[]) =>
      extensions.installExtensions(list)
    );
    this.addAction("updateExtensions", (sync) =>
      extensions.updateExtensions(sync)
    );
    ...
    this.addAction("toggleExtension", (name) =>
      extensions.toggleExtension(name)
    );
    this.addAction("uninstallExtension", (...args: string[]) =>
      extensions.uninstallExtension(args)
    );
    this.addAction("getCurrentFunctionSymbol", () =>
      this.handler.symbols.getCurrentFunctionSymbol()
    );
    this.addAction("showOutline", (keep?: number) =>
      this.handler.symbols.showOutline(keep)
    );
    ...
    this.addAction("addCommand", (cmd) =>
      this.handler.commands.addVimCommand(cmd)
    );
    ...
    this.addAction("selectCurrentPlaceholder", (triggerAutocmd) =>
      snippetManager.selectCurrentPlaceholder(!!triggerAutocmd)
    );
    this.addAction("codeActionRange", (start, end, only) =>
      this.handler.codeActions.codeActionRange(start, end, only)
    );
    this.addAction("incomingCalls", (item?: CallHierarchyItem) =>
      this.handler.callHierarchy.getIncoming(item)
    );
    this.addAction("outgoingCalls", (item?: CallHierarchyItem) =>
      this.handler.callHierarchy.getOutgoing(item)
    );
    this.addAction("showIncomingCalls", () =>
      this.handler.callHierarchy.showCallHierarchyTree("incoming")
    );
    this.addAction("showOutgoingCalls", () =>
      this.handler.callHierarchy.showCallHierarchyTree("outgoing")
    );
    this.addAction("semanticHighlight", () =>
      this.handler.semanticHighlighter.highlightCurrent()
    );
    this.addAction("showSemanticHighlightInfo", () =>
      this.handler.semanticHighlighter.showHiglightInfo()
    );
  }
  ...
  public async init(): Promise<void> {
    ...
    try {
      ...
      logger.info(
        `coc.nvim ${this.version} initialized with node: ${
          process.version
        } after ${Date.now() - s}ms`
      );
      this.emit("ready");
  ...
  private get version(): string {
    return (
      workspace.version + (typeof REVISION === "string" ? "-" + REVISION : "")
    );
  }
