...
export default class InstallBuffer extends EventEmitter implements Disposable {
  ...
  private getLines(): string[] {
    ...
    for (let name of this.names) {
      ...
      lines.push(
        `- ${processText} ${name} ${msgs.length ? msgs[msgs.length - 1] : ""}`
      );
    }
    return lines;
  }
  ...
  private draw(nvim: Neovim, buffer: Buffer): void {
    ...
    let first =
      remains == 0
        ? `${this.isUpdate ? "Update" : "Install"} finished`
        : `Installing, ${remains} remains...`;
  ...
  public async show(nvim: Neovim): Promise<void> {
    ...
    nvim.command(
      "setl buftype=nofile bufhidden=wipe noswapfile nobuflisted wrap undolevels=-1",
      true
    );
    if (!isSync) {
