...
function registryUrl(scope = "coc.nvim"): string {
  ...
  const registry =
    ...
    result.config_registry ||
    (result.registry as string);
...
export class Installer extends EventEmitter {
  ...
  public async install(): Promise<string> {
    ...
    let { name } = info;
    let required = info["engines.coc"]
      ? info["engines.coc"].replace(/^\^/, ">=")
      : "";
    if (required && !semver.satisfies(workspace.version, required)) {
      throw new Error(
        `${name} ${info.version} requires coc.nvim >= ${required}, please update coc.nvim.`
      );
    }
  ...
  public async update(url?: string): Promise<string> {
    ...
    if (required && !semver.satisfies(workspace.version, required)) {
      throw new Error(
        `${info.version} requires coc.nvim ${required}, please update coc.nvim.`
      );
    }
    await this.doInstall(info);
  ...
  private async doInstall(info: Info): Promise<void> {
    ...
    let tmpFolder = await fs.mkdtemp(
      path.join(os.tmpdir(), `${info.name.replace("/", "-")}-`)
    );
    ...
    if (dependencies && Object.keys(dependencies).length) {
      let p = new Promise<void>((resolve, reject) => {
        ...
        if (
          (this.npm.endsWith("npm") || this.npm.endsWith("npm.CMD")) &&
          !this.npm.endsWith("pnpm")
        ) {
    ...
    let obj = parse(fs.readFileSync(jsonFile, "utf8"), errors, {
    ...
    await fs.writeFile(jsonFile, JSON.stringify(sortedObj, null, 2), {
      encoding: "utf8",
    });
  ...
  private async getInfo(): Promise<Info> {
    ...
    let buffer = await fetch(registry + this.name, {
      timeout: 10000,
      buffer: true,
    });
    ...
    if (!requiredVersion) {
      throw new Error(
        `${this.def} is not valid coc extension, "engines" field with coc property required.`
      );
    }
    return {
...
export function createInstallerFactory(
  npm: string,
  root: string
): (def: string) => Installer {
