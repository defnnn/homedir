...
export default class FloatFactory implements Disposable {
  ...
  private bindEvents(autoHide: boolean, alignTop: boolean): void {
    ...
    events.on(
      "CursorMoved",
      this.onCursorMoved.bind(this, autoHide),
      this,
      this.disposables
    );
    events.on(
  ...
  private async createPopup(
    ...
    this.unbind();
    let arr = await this.nvim.call("coc#float#create_cursor_float", [
      this.winid,
      this._bufnr,
      lines,
      config,
    ...
    let [targetBufnr, cursor, winid, bufnr, alignTop] = arr as [
      number,
      [number, number],
      number,
      number,
