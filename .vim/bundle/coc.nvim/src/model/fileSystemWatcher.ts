...
export default class FileSystemWatcher implements Disposable {
  ...
  private async listen(client: Watchman): Promise<Disposable> {
    ...
    let disposable = await client.subscribe(
      globPattern,
      (change: FileChange) => {
        let { root, files } = change;
        files = files.filter(
          (f) => f.type == "f" && minimatch(f.name, globPattern, { dot: true })
        );
        ...
        if (files.length >= 2) {
          let [oldFiles, newFiles] = splitArray(
            files,
            (o) => o.exists === false
          );
          if (oldFiles.length == newFiles.length) {
            for (let oldFile of oldFiles) {
              let newFile = newFiles.find(
                (o) => o.size == oldFile.size && o.mtime_ms == oldFile.mtime_ms
              );
              if (newFile) {
                this._onDidRename.fire({
                  oldUri: URI.file(path.join(root, oldFile.name)),
