...
export default class Document {
  ...
  public readonly onDocumentChange: Event<DidChangeTextDocumentParams> =
    this._onDocumentChange.event;
  public readonly onDocumentDetach: Event<number> =
    this._onDocumentDetach.event;
  constructor(
  ...
  public async init(nvim: Neovim, token: CancellationToken): Promise<boolean> {
    this.nvim = nvim;
    let opts: BufferOption = await nvim.call("coc#util#get_bufoptions", [
      this.bufnr,
      this.maxFileSize,
    ]);
  ...
  public async applyEdits(edits: TextEdit[]): Promise<void> {
    ...
    let textDocument = TextDocument.create(
      this.uri,
      this.filetype,
      1,
  ...
  private createTextDocument(
    ...
    this._textDocument = new LinesTextDocument(
      uri,
      filetype,
      version,
      lines,
  ...
  public async patchChange(currentLine?: boolean): Promise<void> {
    ...
    if (this.env.isVim) {
      if (currentLine) {
        let change = (await this.nvim.call(
          "coc#util#get_changeinfo",
          []
        )) as ChangeInfo;
  ...
  public getSymbolRanges(word: string): Range[] {
    ...
    let textDocument = new LinesTextDocument(
      uri,
      filetype,
      version,
      this.lines,
    ...
    for (let i = 0, l = content.length; i < l; i++) {
      ...
      if (str.length > 0 && !isKeyword && str == word) {
        res.push(
          Range.create(
            textDocument.positionAt(i - str.length),
            textDocument.positionAt(i)
          )
        );
      }
      if (!isKeyword) {
  ...
  public addHighlights(
    ...
    for (let line = start.line; line <= end.line; line++) {
      ...
      let colEnd =
        line == end.line
          ? byteIndex(text, end.character)
          : global.Buffer.byteLength(text);
  ...
  public getEndOffset(lnum: number, col: number, insert: boolean): number {
    ...
    for (let i = lnum - 1; i < len; i++) {
      ...
      if (i == lnum - 1 && l != 0) {
        ...
        if (!isEnd) {
          total =
            total + buf.slice(col - 1, buf.length).toString("utf8").length;
  ...
  public setIskeyword(iskeyword: string): void {
    ...
    let lines =
      this.lines.length > 30000 ? this.lines.slice(0, 30000) : this.lines;
