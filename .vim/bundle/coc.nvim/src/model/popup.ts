...
export default class Popup {
  ...
  public async scrollForward(): Promise<void> {
    ...
    if (!isVim) {
      ...
      botline = (await nvim.eval(
        `get(popup_getpos(${winid}), 'lastline', 0)`
      )) as number;
    }
  ...
  public async scrollBackward(): Promise<void> {
    ...
    if (!isVim) {
      ...
      topline = (await nvim.eval(
        `get(popup_getpos(${winid}), 'firstline', 0)`
      )) as number;
    }
    if (topline == 1) return;
    nvim.pauseNotification();
  ...
  public setCursor(index: number): void {
    ...
    if (isVim) {
      ...
      nvim.command(
        `sign place 6 line=${index + 1} name=CocCurrentLine buffer=${bufnr}`,
        true
      );
    }
  }
