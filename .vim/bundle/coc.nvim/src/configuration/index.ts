...
export default class Configurations {
  ...
  public readonly onDidChange: Event<ConfigurationChangeEvent> =
  ...
  public changeConfiguration(
    ...
    let changed = getChangedKeys(
      this._configuration.getValue(),
      configuration.getValue()
    );
    if (target == ConfigurationTarget.Workspace && configFile) {
      this._folderConfigurations.set(
        configFile,
        new ConfigurationModel(model.contents)
      );
    ...
    this._onChange.fire({
      affectsConfiguration: (section, resource) => {
        ...
        let preRoot = workspaceConfigFile
          ? path.resolve(workspaceConfigFile, "../..")
          : "";
        if (
          configFile &&
          !isParentFolder(preRoot, filepath, true) &&
          !isParentFolder(path.resolve(configFile, "../.."), filepath)
        ) {
          return false;
        }
        return changed.includes(section);
  ...
  public setFolderConfiguration(uri: string): void {
    ...
    for (let [configFile, model] of this.foldConfigurations) {
      ...
      if (
        isParentFolder(root, filepath, true) &&
        this.workspaceConfigFile != configFile
      ) {
  ...
  public getConfiguration(
    ...
    if (resource) {
      ...
      configuration = new Configuration(
        defaults,
        user,
        this.getFolderConfiguration(resource)
      );
    } else {
    ...
    const result: WorkspaceConfiguration = {
      ...
      update: (key: string, value: any, isUser = false) => {
        ...
        if (
          ...
          !this.workspaceConfigFile &&
          this._proxy
        ) {
          let file = (this.workspaceConfigFile =
            this._proxy.workspaceConfigFile);
  ...
  private static parse(data: IConfigurationData): Configuration {
    ...
    const workspaceConfiguration = new ConfigurationModel(
      data.workspace.contents
    );
