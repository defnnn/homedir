...
function asCreateDeleteFilesParams(
  e: FileCreateEvent | FileDeleteEvent
...
function asRenameFilesParams(
  e: FileRenameEvent | FileWillRenameEvent
): RenameFilesParams {
  return {
    files: e.files.map((f) => ({
  ...
  implements DynamicFeature<FileOperationRegistrationOptions>
{
  ...
  public register(
    data: RegistrationData<FileOperationRegistrationOptions>
  ): void {
    if (!this._listener) {
            ...
            if (filter.matcher.match(path)) {
              ...
              if (
                (fileType === FileType.File &&
                  filter.kind === FileOperationPatternKind.file) ||
                (fileType === FileType.Directory &&
                  filter.kind === FileOperationPatternKind.folder)
              ) {
                return true;
              }
  ...
  private static asMinimatchOptions(
    options: FileOperationPatternOptions | undefined
  ): minimatch.IOptions | undefined {
...
> extends FileOperationFeature<I, E> {
  private _notificationType: ProtocolNotificationType<
    P,
    FileOperationRegistrationOptions
  >;
  ...
  constructor(
    client: BaseLanguageClient,
    event: Event<E>,
    notificationType: ProtocolNotificationType<
      P,
      FileOperationRegistrationOptions
    >,
    clientCapability: keyof FileOperationClientCapabilities,
...
> {
  ...
  protected doSend(
    ...
    next: (event: FileCreateEvent) => void
  ): void {
...
export class DidRenameFilesFeature extends NotificationFileOperationFeature<
...
> {
  ...
  protected doSend(
    ...
    next: (event: FileRenameEvent) => void
  ): void {
    const middleware = this._client.clientOptions.middleware?.workspace;
...
export class DidDeleteFilesFeature extends NotificationFileOperationFeature<
  URI,
  FileDeleteEvent,
  DeleteFilesParams
> {
...
abstract class RequestFileOperationFeature<
  ...
  P
> extends FileOperationFeature<I, E> {
  private _requestType: ProtocolRequestType<
    P,
    WorkspaceEdit | null,
    never,
    void,
  ...
  protected abstract doSend(
    event: E,
    next: (event: EventWithFiles<I>) => Thenable<WorkspaceEdit> | Thenable<any>
...
> {
  ...
  protected doSend(
    ...
    next: (event: FileCreateEvent) => Thenable<WorkspaceEdit> | Thenable<any>
  ): Thenable<WorkspaceEdit> | Thenable<any> {
    const middleware = this._client.clientOptions.middleware?.workspace;
...
> {
  ...
  protected doSend(
    ...
    next: (event: FileRenameEvent) => Thenable<WorkspaceEdit> | Thenable<any>
  ): Thenable<WorkspaceEdit> | Thenable<any> {
    const middleware = this._client.clientOptions.middleware?.workspace;
...
> {
  ...
  protected doSend(
    ...
    next: (event: FileDeleteEvent) => Thenable<WorkspaceEdit> | Thenable<any>
  ): Thenable<WorkspaceEdit> | Thenable<any> {
    const middleware = this._client.clientOptions.middleware?.workspace;
