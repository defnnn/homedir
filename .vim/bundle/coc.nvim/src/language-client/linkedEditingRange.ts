...
export class LinkedEditingFeature extends TextDocumentFeature<
...
> {
  ...
  protected registerLanguageProvider(
    options: LinkedEditingRangeRegistrationOptions
    ...
    const provider: LinkedEditingRangeProvider = {
      provideLinkedEditingRanges: (document, position, token) => {
        ...
        const provideLinkedEditing: ProvideLinkedEditingRangeSignature = (
          ...
          return client
            .sendRequest(LinkedEditingRangeRequest.type, params, token)
            .then(
              (result) => result,
              (error) => {
