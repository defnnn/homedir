...
export type ServerOptions =
  ...
  | NodeModule
  | (() => Promise<
      ChildProcess | StreamInfo | MessageTransports | ChildProcessInfo
    >);
...
export class LanguageClient extends BaseLanguageClient {
  ...
  protected createMessageTransports(
    ...
    return this._getServerWorkingDir(json.options).then((serverWorkingDir) => {
      if (NodeModule.is(json) && json.module) {
        ...
        if (node.runtime) {
          ...
          return new Promise<MessageTransports>((resolve, _reject) => {
            ...
            if (
              ...
              void createClientSocketTransport(transport.port).then(
                (transport) => {
                  let sp = cp.fork(node.module, args || [], options);
                  assertStdio(sp);
                  this._serverProcess = sp;
        ...
        if (!serverProcess || !serverProcess.pid) {
          return Promise.reject<MessageTransports>(
            `Launching server "${this.id}" using command ${command.command} failed.`
          );
        }
        logger.info(
          `Language server "${this.id}" started with ${serverProcess.pid}`
        );
      ...
      return Promise.reject<MessageTransports>(
        `Unsupported server configuration ${JSON.stringify(server, null, 2)}`
      );
    });
  }
  ...
  private _getRuntimePath(
    runtime: string,
    serverWorkingDirectory: string | undefined
  ): string {
