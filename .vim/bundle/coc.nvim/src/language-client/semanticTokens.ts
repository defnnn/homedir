...
export class SemanticTokensFeature extends TextDocumentFeature<
...
> {
  ...
  protected registerLanguageProvider(
  ...
  ): [Disposable, SemanticTokensProviders] {
    ...
    const documentProvider: DocumentSemanticTokensProvider | undefined =
      fullProvider
        ? {
            onDidChangeSemanticTokens: eventEmitter.event,
            provideDocumentSemanticTokens: (document, token) => {
              const client = this._client;
              const middleware = client.clientOptions
                .middleware! as Middleware & SemanticTokensMiddleware;
    ...
    return [
      Disposable.create(() => disposables.forEach((item) => item.dispose())),
      {
        range: rangeProvider,
