...
class DefaultErrorHandler implements ErrorHandler {
  ...
  public closed(): CloseAction {
    ...
    if (this.restarts.length < this.maxRestartCount) {
      ...
      if (diff <= 3 * 60 * 1000) {
        window.showMessage(
          `The "${this.name}" server crashed ${this.maxRestartCount} times in the last 3 minutes. The server will not be restarted.`,
          "error"
        );
...
const SupportedSymbolTags: SymbolTag[] = [SymbolTag.Deprecated];
...
abstract class DocumentNotifications<P, E>
  implements
    ...
    NotificationFeature<(data: E) => void>
{
  ...
  constructor(
    ...
    protected _type: ProtocolNotificationType<
      P,
      TextDocumentRegistrationOptions
    >,
...
class DidChangeTextDocumentFeature
  implements
    DynamicFeature<TextDocumentChangeRegistrationOptions>,
    NotificationFeature<(event: DidChangeTextDocumentParams) => void>
{
  private _listener: Disposable | undefined;
  private _changeData: Map<string, DidChangeTextDocumentData> = new Map<
    string,
    DidChangeTextDocumentData
  >();
      ...
      if (workspace.match(changeData.documentSelector, textDocument) > 0) {
        ...
        if (changeData.syncKind === TextDocumentSyncKind.Incremental) {
          ...
          let didChange: (event: DidChangeTextDocumentParams) => void = (
            ...
            let { textDocument } = workspace.getDocument(
  ...
  public getProvider(
    document: TextDocument
  ): { send: (event: DidChangeTextDocumentParams) => void } | undefined {
    for (const changeData of this._changeData.values()) {
...
{
  private _listener: Disposable | undefined;
  private _selectors: Map<string, DocumentSelector> = new Map<
    string,
    DocumentSelector
  >();
...
> {
  ...
  constructor(client: BaseLanguageClient) {
    super(
      ...
      (textDocument) =>
        cv.asSaveTextDocumentParams(textDocument, this._includeText),
      DocumentNotifications.textDocumentFilter
    );
  ...
  public fillClientCapabilities(capabilities: ClientCapabilities): void {
    ensure(ensure(capabilities, "textDocument")!, "synchronization")!.didSave =
      true;
  }
...
{
  private _watchers: Map<string, Disposable[]> = new Map<
    string,
    Disposable[]
  >();
...
{
  private _registrations: Map<string, TextDocumentFeatureRegistration<RO, PR>> =
    new Map();
  ...
  protected getRegistration(
    ...
    if (!capability) {
      ...
      let options: RO & { documentSelector: DocumentSelector } = (
        Is.boolean(capability) && capability === true
          ? { documentSelector }
...
abstract class WorkspaceFeature<RO, PR> implements DynamicFeature<RO> {
  protected _registrations: Map<string, WorkspaceFeatureRegistration<PR>> =
    new Map();
...
export interface ProvideResolveFeature<
  T1 extends Function,
  T2 extends Function
> {
  provide: T1;
...
> {
  ...
  public fillClientCapabilities(capabilities: ClientCapabilities): void {
    let snippetSupport =
      this._client.clientOptions.disableSnippetCompletion !== true;
      ...
      resolveCompletionItem: options.resolveProvider
        ? (
            ...
            const resolveCompletionItem: ResolveCompletionItemSignature = (
              ...
              return client
                .sendRequest(CompletionResolveRequest.type, item, token)
                .then(
                  (res) => res,
                  (error) => {
                    return client.handleFailedRequest(
...
> {
  ...
  protected registerLanguageProvider(
    ...
    return [
      languages.registerHoverProvider(options.documentSelector!, provider),
      provider,
    ];
  }
}
...
> {
  ...
  protected registerLanguageProvider(
    ...
    const disposable = languages.registerSignatureHelpProvider(
...
> {
  ...
  protected registerLanguageProvider(
    ...
    return [
      languages.registerDefinitionProvider(options.documentSelector!, provider),
      provider,
    ];
  }
}
...
> {
  ...
  protected registerLanguageProvider(
    ...
    return [
      languages.registerReferencesProvider(options.documentSelector!, provider),
      provider,
    ];
  }
}
...
> {
  ...
  protected registerLanguageProvider(
    _options: WorkspaceSymbolRegistrationOptions
  ...
  protected registerLanguageProvider(
    ...
    const provider: CodeActionProvider = {
      provideCodeActions: (document, range, context, token) => {
        const client = this._client;
        const _provideCodeActions: ProvideCodeActionsSignature = (
          document,
          range,
          context,
          token
      ...
      resolveCodeAction: options.resolveProvider
        ? (item: CodeAction, token: CancellationToken) => {
            ...
            const resolveCodeAction: ResolveCodeActionSignature = (
              ...
              return client
                .sendRequest(CodeActionResolveRequest.type, item, token)
                .then(
                  (values) => values,
                  (error) => {
                    return client.handleFailedRequest(
...
> {
  ...
  protected registerLanguageProvider(
      ...
      resolveCodeLens: options.resolveProvider
        ? (
            ...
            const resolveCodeLens: ResolveCodeLensSignature = (
              ...
              return client
                .sendRequest(CodeLensResolveRequest.type, codeLens, token)
                .then(
                  (res) => res,
                  (error) => {
                    return client.handleFailedRequest(
    ...
    return [
      languages.registerCodeLensProvider(options.documentSelector, provider),
      { provider, onDidChangeCodeLensEmitter: emitter },
    ];
  }
}
  ...
  protected registerLanguageProvider(
    ...
    return [
      languages.registerDocumentFormatProvider(
        options.documentSelector!,
        provider,
        this._client.clientOptions.formatterPriority
      ),
      provider,
    ];
  }
...
> {
  ...
  protected registerLanguageProvider(
    options: RenameRegistrationOptions
      ...
      prepareRename: options.prepareProvider
        ? (document, position, token) => {
            ...
            const prepareRename: PrepareRenameSignature = (
              ...
              return client
                .sendRequest(PrepareRenameRequest.type, params, token)
                .then(
                  (result) => {
                    if (Range.is(result)) {
                      return result;
    ...
    return [
      languages.registerRenameProvider(options.documentSelector, provider),
      provider,
    ];
  }
  ...
  protected registerLanguageProvider(
    ...
    const provider: DocumentLinkProvider = {
      ...
      resolveDocumentLink: options.resolveProvider
        ? (link, token) => {
            const client = this._client;
            let resolveDocumentLink: ResolveDocumentLinkSignature = (
              link,
              token
            ) => {
              return client
                .sendRequest(DocumentLinkResolveRequest.type, link, token)
                .then(
                  (res) => res,
                  (error: ResponseError<void>) => {
                    return client.handleFailedRequest(
  ...
  implements DynamicFeature<DidChangeConfigurationRegistrationOptions>
{
  ...
  public register(
    ...
    let disposable = workspace.onDidChangeConfiguration((event) => {
      ...
      if (
        ...
        !section.some((v) => event.affectsConfiguration(v))
      ) {
        return;
      }
      if (section != undefined) {
  ...
  private onDidChangeConfiguration(
    ...
    let isConfigured =
      typeof configurationSection === "string" &&
      configurationSection.startsWith("languageserver.");
    ...
    let didChangeConfiguration = (sections: string[] | undefined): void => {
      ...
      this._client.sendNotification(DidChangeConfigurationNotification.type, {
        settings: isConfigured
          ? this.getConfiguredSettings(sections[0])
          : this.extractSettingsInformation(sections),
      });
    };
  ...
  private extractSettingsInformation(keys: string[]): any {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      if (index >= 0) {
        config = workspace
          .getConfiguration(key.substr(0, index))
...
export abstract class BaseLanguageClient {
  ...
  public constructor(
    ...
    if (
      suggest.get<boolean>("snippetsSupport", true) === false ||
      clientOptions.disableSnippetCompletion
    ) {
    ...
    if (
      clientOptions.markdown !== undefined &&
      clientOptions.markdown.isTrusted === true
    ) {
  ...
  private _appendOutput(type: string, message: string, data?: any): void {
    ...
    this.outputChannel.appendLine(
      `[${type}  - ${new Date().toLocaleTimeString()}] ${message}`
    );
  ...
  public start(): Disposable {
    ...
    if (!this._diagnostics) {
      ...
      let name = opts.diagnosticCollectionName
        ? opts.diagnosticCollectionName
        : this._id;
  ...
  private resolveRootPath(): string | null | false {
    ...
    if (
      rootPath === os.homedir() ||
      (ignoredRootPaths && ignoredRootPaths.includes(rootPath))
    ) {
  ...
  private initialize(connection: IConnection): Promise<InitializeResult> {
    ...
    let { initializationOptions, progressOnInitialization } =
      this._clientOptions;
    ...
    let initParams: any = {
      ...
      initializationOptions: Is.func(initializationOptions)
  ...
  public stop(): Promise<void> {
    ...
    this.cleanUp();
    // unkook listeners
    return (this._onStop = this.resolveConnection()
      .then((connection) => {
  ...
  private setDiagnostics(uri: string, diagnostics: Diagnostic[] | undefined) {
    ...
    const separate = workspace
      .getConfiguration("diagnostic")
      .get("separateRelatedInformationAsDiagnostics") as boolean;
  ...
  public getFeature(
    ...
    NotificationFeature<(textDocument: TextDocument) => void>;
  public getFeature(
  ...
  public getFeature(
    ...
    TextDocumentProviderFeature<FoldingRangeProvider>;
  public getFeature(
  ...
  public getFeature(
    ...
    TextDocumentProviderFeature<SemanticTokensProviders>;
  public getFeature(
