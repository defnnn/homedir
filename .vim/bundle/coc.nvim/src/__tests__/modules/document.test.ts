...
describe("properties", () => {
  ...
  it("should not attach change events when b:coc_enabled is false", async () => {
    await nvim.command(
      "autocmd BufNewFile,BufRead *.dis let b:coc_enabled = 0"
    );
...
describe("synchronize", () => {
  ...
  it("should synchronize changes after applyEdits", async () => {
    ...
    await document.applyEdits([
      TextEdit.insert({ line: 0, character: 0 }, "d"),
    ]);
...
describe("applyEdits", () => {
  it("should synchronize content added", async () => {
    ...
    await doc.buffer.setLines(["foo f"], {
      start: 0,
      end: -1,
      strictIndexing: false,
    });
...
describe("highlights", () => {
  it("should add highlights to document", async () => {
    ...
    await buf.setLines(["你好", "world"], {
      start: 0,
      end: -1,
      strictIndexing: false,
    });
    let ranges = [Range.create(0, 0, 0, 2), Range.create(1, 0, 1, 3)];
