...
describe("Position", () => {
  test("rangeInRange", () => {
    ...
    expect(rangeInRange(r, Range.create(addPosition(pos, 1, 0), pos))).toBe(
      false
    );
  });
  ...
  test("rangeOverlap", () => {
    ...
    expect(
      rangeOverlap(Range.create(0, 0, 0, 10), Range.create(0, 1, 0, 2))
    ).toBe(true);
    expect(
      rangeOverlap(Range.create(0, 0, 0, 1), Range.create(0, 1, 0, 2))
    ).toBe(false);
    expect(
      rangeOverlap(Range.create(0, 1, 0, 2), Range.create(0, 0, 0, 1))
    ).toBe(false);
    expect(
      rangeOverlap(Range.create(0, 0, 0, 1), Range.create(0, 2, 0, 3))
    ).toBe(false);
  });
