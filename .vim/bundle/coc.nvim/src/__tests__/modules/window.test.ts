...
describe("window functions", () => {
  it("should get offset", async () => {
    ...
    await doc.applyEdits([
      { range: Range.create(0, 0, 0, 0), newText: "foo\nbar" },
    ]);
  ...
  it("should open temimal buffer", async () => {
    let bufnr = await window.openTerminal("ls", {
      autoclose: false,
      keepfocus: false,
    });
  ...
  it("should show information message", async () => {
    let p = window.showInformationMessage(
      "information message",
      "first",
      "second"
    );
...
describe("window notifications", () => {
  ...
  it("should show progress notification", async () => {
    ...
    let res = await window.withProgress(
      { title: "Downloading", cancellable: true },
      (progress, token) => {
        let n = 0;
