...
async function createDocument(name?: string): Promise<Document> {
  ...
  diagnostics.push(
    createDiagnostic("hint", Range.create(1, 2, 1, 3), DiagnosticSeverity.Hint)
  );
  diagnostics.push(
    createDiagnostic(
      "error",
...
describe("diagnostic manager", () => {
  ...
  describe("getDiagnosticsInRange", () => {
    it("should get diagnostics in range", async () => {
      ...
      collection.set(doc.uri, diagnostics);
      let res = manager.getDiagnosticsInRange(
        doc.textDocument,
    ...
    it("should filter diagnostics by level", async () => {
      ...
      for (let diagnostics of Object.values(diagnosticsMap)) {
        for (let diagnostic of diagnostics) {
          ...
          expect(diagnostic.severity != DiagnosticSeverity.Information).toBe(
            true
          );
        }
      }
  ...
  describe("refresh", () => {
    ...
    it("should refresh all buffers", async () => {
      ...
      let collection = manager.create("tmp");
      collection.set([
        [one.uri, [createDiagnostic("Error one")]],
        [two.uri, [createDiagnostic("Error two")]],
      ]);
      manager.refresh();
      await helper.wait(50);
