...
describe("workspace properties", () => {
  it("should have initialized", () => {
    let { nvim, rootPath, cwd, documents, initialized, textDocuments } =
      workspace;
...
describe("workspace applyEdits", () => {
  it("should apply TextEdit of documentChanges", async () => {
    ...
    let versioned = VersionedTextDocumentIdentifier.create(
      doc.uri,
      doc.version
    );
...
describe("workspace methods", () => {
  ...
  it("should return match score for document", async () => {
    ...
    expect(workspace.match([{ pattern: "**/*.xml" }], doc.textDocument)).toBe(
      5
    );
    expect(workspace.match([{ pattern: "**/*.html" }], doc.textDocument)).toBe(
      0
    );
...
describe("workspace utility", () => {
  ...
  it("should loadFiles", async () => {
    let files = ["a", "b", "c"].map((key) =>
      URI.file(path.join(__dirname, key)).toString()
    );
