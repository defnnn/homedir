...
describe("completion start", () => {
  it("should deactivate on doComplete error", async () => {
    ...
    let option: CompleteOption = await nvim.call(
...
describe("completion TextChangedP", () => {
  ...
  it("should fix cursor position with plain text on additionalTextEdits", async () => {
    ...
    disposables.push(
      languages.registerCompletionItemProvider("edits", "edit", null, provider)
    );
    await nvim.input("if");
    await helper.waitPopup();
  ...
  it("should adjust completion position by textEdit start position", async () => {
    ...
    disposables.push(
      languages.registerCompletionItemProvider("fix", "f", null, provider, [
        "?",
      ])
    );
    await nvim.input("i?");
  ...
  it("should filter on none keyword input", async () => {
    let source: ISource = {
      ...
      sourceType: SourceType.Service,
      doComplete: (_opt: CompleteOption): Promise<CompleteResult> =>
        Promise.resolve({ items: [{ word: "foo#abc" }] }),
    };
    disposables.push(sources.addSource(source));
    await nvim.input("if");
    await helper.waitPopup();
  ...
  it("should do resolve for complete item", async () => {
    let source: ISource = {
      ...
      triggerCharacters: ["."],
      doComplete: (_opt: CompleteOption): Promise<CompleteResult> =>
        Promise.resolve({ items: [{ word: "foo" }] }),
