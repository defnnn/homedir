...
describe("refactor", () => {
  ...
  describe("onChange", () => {
    it("should ignore when change after range", async () => {
      ...
      await refactor.fromLocations([
        { uri: doc.uri, range: Range.create(0, 0, 0, 3) },
      ]);
    ...
    it("should adjust when change before range", async () => {
      ...
      let buf = await refactor.fromLocations([
        { uri: doc.uri, range: Range.create(4, 0, 4, 3) },
      ]);
      await doc.buffer.setLines(["def"], {
        start: 0,
        end: 0,
