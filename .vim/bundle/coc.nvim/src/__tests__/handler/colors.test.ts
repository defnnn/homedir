    ...
    languages.registerDocumentColorProvider([{ language: "*" }], {
      ...
      provideDocumentColors: (
        ...
        let matches = Array.from(
          (document.getText() as any).matchAll(/#\w{6}/g)
        ) as any;
...
describe("Colors", () => {
  ...
  describe("commands", () => {
    ...
    it("should register editor.action.colorPresentation command", async () => {
      colorPresentations = [
        ColorPresentation.create("red"),
        ColorPresentation.create("#ff0000"),
      ];
      let doc = await helper.createDocument();
      await nvim.setLine("#ffffff");
      doc.forceSync();
  ...
  describe("doHighlight", () => {
    ...
    it("should highlight after document changed", async () => {
      ...
      expect(colors.hasColorAtPosition(doc.bufnr, Position.create(0, 1))).toBe(
        false
      );
