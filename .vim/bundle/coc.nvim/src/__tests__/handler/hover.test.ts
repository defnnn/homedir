...
async function getDocumentText(): Promise<string> {
  let lines = (await nvim.call("getbufline", [
    "coc://document",
    1,
    "$",
  ])) as string[];
    ...
    it("should show MarkedString hover", async () => {
      ...
      disposables.push(
        languages.registerHoverProvider([{ language: "*" }], {
          provideHover: (_doc, _pos, _token) => {
            return {
              contents: { language: "typescript", value: "language hover" },
            };
          },
        })
      );
    ...
    it("should get hover from MarkedString array", async () => {
      ...
      disposables.push(
        languages.registerHoverProvider([{ language: "*" }], {
          provideHover: (_doc, _pos, _token) => {
            return {
              contents: MarkedString.fromPlainText("MarkedString hover"),
            };
          },
        })
      );
