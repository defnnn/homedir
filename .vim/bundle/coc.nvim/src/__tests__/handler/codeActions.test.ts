...
describe("handler codeActions", () => {
  ...
  describe("getCodeActions", () => {
        ...
        languages.registerCodeActionProvider(
          [{ language: "*" }],
          {
            provideCodeActions: (
              ...
              range = r;
              if (rangeInRange(r, Range.create(0, 0, 1, 0)))
    ...
    it("should filter actions by kind prefix", async () => {
      ...
      currActions = [action];
      let res = await codeActions.getCodeActions(doc, undefined, [
  ...
  describe("doQuickfix", () => {
    ...
    it("should do preferred quickfix action", async () => {
      ...
      currActions = [
        CodeAction.create("foo", CodeActionKind.QuickFix),
        action,
        CodeAction.create("bar"),
      ];
      await codeActions.doQuickfix();
      let lines = await doc.buffer.lines;
      expect(lines).toEqual(["bar"]);
  ...
  describe("applyCodeAction", () => {
    it("should resolve codeAction", async () => {
      ...
      let arr = await codeActions.getCurrentCodeActions("line", [
    ...
    it("should invoke registered command after apply edit", async () => {
      let called;
      disposables.push(
        commands.registerCommand("test.execute", async (s: string) => {
          called = s;
