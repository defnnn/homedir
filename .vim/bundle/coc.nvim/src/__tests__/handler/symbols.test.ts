...
beforeEach(() => {
  disposables.push(
    languages.registerDocumentSymbolProvider([{ language: "javascript" }], {
...
describe("symbols handler", () => {
  ...
  describe("selectSymbolRange", () => {
    ...
    it("should select symbol range at cursor position", async () => {
      ...
      let buf = await createBuffer(code);
      await nvim.call("cursor", [3, 0]);
      await helper.doAction("selectSymbolRange", false, "", [
        "Function",
        "Method",
      ]);
  ...
  describe("cancel", () => {
    it("should cancel symbols request on insert", async () => {
      let cancelled = false;
      disposables.push(
        languages.registerDocumentSymbolProvider([{ language: "text" }], {
