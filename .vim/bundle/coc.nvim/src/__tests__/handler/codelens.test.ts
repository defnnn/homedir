...
describe("codeLenes featrue", () => {
  it("should do codeLenes request and resolve codeLenes", async () => {
    disposables.push(
      languages.registerCodeLensProvider([{ language: "javascript" }], {
        provideCodeLenses: () => {
          return [
            {
  ...
  it("should invoke codeLenes action", async () => {
    let fn = jest.fn();
    disposables.push(
      commands.registerCommand("__save", (...args) => {
        fn(...args);
      })
    );
    disposables.push(
  ...
  it("should use picker fo multiple codeLenses", async () => {
    ...
    disposables.push(
      commands.registerCommand("__delete", (...args) => {
        fn(...args);
      })
    );
    disposables.push(
  ...
  it("should refresh for failed codeLens request", async () => {
    ...
    disposables.push(
      commands.registerCommand("__foo", (...args) => {
        fn(...args);
      })
    );
    disposables.push(
