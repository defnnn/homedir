...
describe("CallHierarchy", () => {
  ...
  it("should get undefined when prepare failed", async () => {
    disposables.push(
      languages.registerCallHierarchyProvider([{ language: "*" }], {
  ...
  it("should render description and support default action", async () => {
    ...
    let buf = await nvim.buffer;
    let lines = await buf.lines;
    expect(lines).toEqual(["INCOMING CALLS", "- c foo", "  + c bar Detail"]);
  ...
  it("should invoke open in new tab action", async () => {
    ...
    let buf = await nvim.buffer;
    let lines = await buf.lines;
    expect(lines).toEqual(["OUTGOING CALLS", "- c foo", "  + c bar Detail"]);
    await nvim.command("exe 3");
  ...
  it("should invoke show incoming calls action", async () => {
    ...
    expect(lines).toEqual(["INCOMING CALLS", "- c bar Detail", "  + c test"]);
  ...
  it("should invoke show outgoing calls action", async () => {
    ...
    let buf = await nvim.buffer;
    let lines = await buf.lines;
    expect(lines).toEqual(["INCOMING CALLS", "- c foo", "  + c test"]);
    await nvim.command("exe 3");
    await nvim.input("<tab>");
    await helper.wait(50);
    await nvim.input("3");
    await helper.wait(200);
    lines = await buf.lines;
    expect(lines).toEqual(["OUTGOING CALLS", "- c test", "  + c bar Detail"]);
    await nvim.command("bd!");
  });
  ...
  it("should invoke dismiss action #1", async () => {
    ...
    await helper.wait(200);
    lines = await buf.lines;
    expect(lines).toEqual(["OUTGOING CALLS", "- c foo"]);
