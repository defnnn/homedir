...
import BasicDataProvider, {
  ProviderOptions,
  TreeNode,
} from "../../tree/BasicDataProvider";
...
function createTreeView(
  defs: NodeDef[],
  opts: Partial<TreeViewOptions<TreeNode>> = {},
  providerOpts: Partial<ProviderOptions<TreeNode>> = {}
) {
  nodes = createNodes(defs);
  provider = new BasicDataProvider(
    Object.assign(providerOpts, {
      provideData: () => {
        return nodes;
      },
    })
...
describe("TreeView", () => {
  ...
  describe("options", () => {
    ...
    it("should support many selection", async () => {
      ...
      let res = await nvim.call("sign_getplaced", [
        buf.id,
        { group: "CocTree" },
      ]);
  ...
  describe("key-mappings", () => {
    ...
    it("should toggle expand by t", async () => {
      ...
      await nvim.input("t");
      await helper.wait(50);
      await checkLines(["test", "+ a", "- b", "    e", "    f", "  g"]);
    });
  ...
  describe("data change", () => {
    ...
    it("should render changed nodes", async () => {
      ...
      await helper.wait(50);
      await checkLines(["test", "- a", "  + h", "    d", "+ b", "  g"]);
      let res = await treeView.checkLines();
      expect(res).toBe(true);
    });
  ...
  describe("filter", () => {
    async function createFilterTreeView(
      opts: Partial<ProviderOptions<TreeNode>> = {}
    ): Promise<void> {
    ...
    it("should add & remove Cursor highlight on window change", async () => {
      ...
      await helper.wait(50);
      markers = (await nvim.call("nvim_buf_get_extmarks", [
        bufnr,
        ns,
        [1, 0],
        [1, -1],
