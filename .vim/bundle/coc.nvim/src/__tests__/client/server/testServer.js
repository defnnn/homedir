...
connection.onInitialize((params) => {
  ...
  assert.equal(
    params.capabilities.textDocument.completion.completionItem.preselectSupport,
    true
  );
  assert.equal(
  ...
  assert.equal(
    params.capabilities.textDocument.publishDiagnostics.tagSupport.valueSet
      .length,
    2
  );
  assert.equal(
    params.capabilities.textDocument.publishDiagnostics.tagSupport.valueSet[0],
    DiagnosticTag.Unnecessary
  );
  assert.equal(
    params.capabilities.textDocument.publishDiagnostics.tagSupport.valueSet[1],
...
connection.onInitialized(() => {
  ...
  connection.client.register(DidCreateFilesNotification.type, {
    filters: [
      { scheme: "file", pattern: { glob: "**/created-dynamic/**{/,/*.js}" } },
    ],
  });
  connection.client.register(DidRenameFilesNotification.type, {
    filters: [
      ...
      {
        scheme: "file",
        pattern: { glob: "**/renamed-dynamic/**/*.js", matches: "file" },
      },
    ],
  });
  connection.client.register(DidDeleteFilesNotification.type, {
    filters: [
      { scheme: "file", pattern: { glob: "**/deleted-dynamic/**{/,/*.js}" } },
    ],
  });
