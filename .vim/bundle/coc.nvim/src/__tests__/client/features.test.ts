...
describe("Client integration", () => {
  ...
  test("Goto Definition", async () => {
    const provider = client
      .getFeature(DefinitionRequest.method)
  ...
  test("Hover", async () => {
    const provider = client
      .getFeature(HoverRequest.method)
      .getProvider(document);
    isDefined(provider);
  ...
  test("Completion", async () => {
    const provider = client
      .getFeature(CompletionRequest.method)
      .getProvider(document);
    isDefined(provider);
  ...
  test("SignatureHelpRequest", async () => {
    const provider = client
      .getFeature(SignatureHelpRequest.method)
      .getProvider(document);
    isDefined(provider);
  ...
  test("Document Highlight", async () => {
    const provider = client
      .getFeature(DocumentHighlightRequest.method)
      .getProvider(document);
    isDefined(provider);
    ...
    await provider.provideDocumentHighlights(
      document,
      position,
      tokenSource.token
    );
  ...
  test("Code Actions", async () => {
    const provider = client
      .getFeature(CodeActionRequest.method)
      .getProvider(document);
    isDefined(provider);
    ...
    const resolved = await provider.resolveCodeAction(
      result[0],
      tokenSource.token
    );
  ...
  test("Progress", async () => {
    ...
    middleware.handleWorkDoneProgress = (
      token: ProgressToken,
      params,
      next
    ) => {
    ...
    for (let i = 0; i < 2; i++) {
      await new Promise<unknown>((resolve, reject) => {
        ...
        client
          .sendRequest(
            new ProtocolRequestType<any, null, never, any, any>(
  ...
  test("Document Formatting", async () => {
    const provider = client
      .getFeature(DocumentFormattingRequest.method)
      .getProvider(document);
    isDefined(provider);
  ...
  test("Document Range Formatting", async () => {
    const provider = client
      .getFeature(DocumentRangeFormattingRequest.method)
      .getProvider(document);
    isDefined(provider);
  ...
  test("Document on Type Formatting", async () => {
    const provider = client
      .getFeature(DocumentOnTypeFormattingRequest.method)
      .getProvider(document);
    isDefined(provider);
  ...
  test("Rename", async () => {
    const provider = client
      .getFeature(RenameRequest.method)
      .getProvider(document);
    isDefined(provider);
  ...
  test("Document Link", async () => {
    const provider = client
      .getFeature(DocumentLinkRequest.method)
      .getProvider(document);
    isDefined(provider);
  ...
  test("Document Color", async () => {
    const provider = client
      .getFeature(DocumentColorRequest.method)
      .getProvider(document);
    isDefined(provider);
    ...
    await provider.provideColorPresentations(
      color.color,
      { document, range },
      tokenSource.token
    );
  ...
  test("Goto Declaration", async () => {
    const provider = client
      .getFeature(DeclarationRequest.method)
      .getProvider(document);
    isDefined(provider);
  ...
  test("Folding Ranges", async () => {
    const provider = client
      .getFeature(FoldingRangeRequest.method)
      .getProvider(document);
    isDefined(provider);
  ...
  test("Goto Implementation", async () => {
    const provider = client
      .getFeature(ImplementationRequest.method)
      .getProvider(document);
    isDefined(provider);
  ...
  test("Selection Range", async () => {
    const provider = client
      .getFeature(SelectionRangeRequest.method)
      .getProvider(document);
    isDefined(provider);
  ...
  test("Type Definition", async () => {
    const provider = client
      .getFeature(TypeDefinitionRequest.method)
      .getProvider(document);
    isDefined(provider);
  ...
  test("Call Hierarchy", async () => {
    const provider = client
      .getFeature(CallHierarchyPrepareRequest.method)
      .getProvider(document);
    isDefined(provider);
    ...
    const item = result[0];
    const incoming = (await provider.provideCallHierarchyIncomingCalls(
      item,
      tokenSource.token
    ...
    const outgoing = (await provider.provideCallHierarchyOutgoingCalls(
      item,
      tokenSource.token
  ...
  test("File Operations - Will Create Files", async () => {
    ...
    const sendCreateRequest = () =>
      new Promise<WorkspaceEdit>(async (resolve, reject) => {
        ...
        reject(
          new Error("Feature unexpectedly did not call waitUntil synchronously")
        );
      });
    ...
    middleware.workspace.willCreateFiles = (event, next) =>
      next({
        ...event,
  ...
  test("File Operations - Did Create Files", async () => {
    ...
    // Add middleware that strips out any folders.
    middleware.workspace = middleware.workspace || {};
    middleware.workspace.didCreateFiles = (event, next) =>
      next({
        files: event.files.filter((f) => !f.path.endsWith("/")),
      });
  ...
  test("File Operations - Will Rename Files", async () => {
    ...
    // Add middleware that strips out any folders.
    middleware.workspace = middleware.workspace || {};
    middleware.workspace.willRenameFiles = (event, next) =>
      next({
        ...event,
  ...
  test("File Operations - Did Rename Files", async () => {
    ...
    await ensureNotificationReceived("rename", {
      files: [
        {
          oldUri: "file:///my/renamed-static/file.txt",
          newUri: "file:///my-new/renamed-static/file.txt",
        },
        {
          oldUri: "file:///my/renamed-static/folder/",
          newUri: "file:///my-new/renamed-static/folder/",
        },
        {
          oldUri: "file:///my/renamed-dynamic/file.js",
          newUri: "file:///my-new/renamed-dynamic/file.js",
        },
        {
          oldUri: "file:///my/renamed-dynamic/folder/",
          newUri: "file:///my-new/renamed-dynamic/folder/",
        },
      ],
    });
    ...
    // Add middleware that strips out any folders.
    middleware.workspace = middleware.workspace || {};
    middleware.workspace.didRenameFiles = (event, next) =>
      next({
        files: event.files.filter((f) => !f.oldUri.path.endsWith("/")),
      });
  ...
  test("File Operations - Will Delete Files", async () => {
    ...
    // Add middleware that strips out any folders.
    middleware.workspace = middleware.workspace || {};
    middleware.workspace.willDeleteFiles = (event, next) =>
      next({
        ...event,
        files: event.files.filter((f) => !f.path.endsWith("/")),
      });
  ...
  test("File Operations - Did Delete Files", async () => {
    ...
    // Add middleware that strips out any folders.
    middleware.workspace = middleware.workspace || {};
    middleware.workspace.didDeleteFiles = (event, next) =>
      next({
        files: event.files.filter((f) => !f.path.endsWith("/")),
      });
  ...
  test("Semantic Tokens", async () => {
    ...
    const rangeResult = await rangeProvider.provideDocumentRangeSemanticTokens(
      document,
      range,
      tokenSource.token
    );
    ...
    await rangeProvider.provideDocumentRangeSemanticTokens(
      document,
      range,
      tokenSource.token
    );
    ...
    const fullResult = await fullProvider.provideDocumentSemanticTokens(
      document,
      tokenSource.token
    );
    ...
    await fullProvider.provideDocumentSemanticTokens(
      document,
      tokenSource.token
    );
    ...
    await fullProvider.provideDocumentSemanticTokensEdits!(
      document,
      "2",
      tokenSource.token
    );
  ...
  test("Linked Editing Ranges", async () => {
    const provider = client
      .getFeature(LinkedEditingRangeRequest.method)
      .getProvider(document);
    isDefined(provider);
    ...
    await provider.provideLinkedEditingRanges(
      document,
      position,
      tokenSource.token
    );
