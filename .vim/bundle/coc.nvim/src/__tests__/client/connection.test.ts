...
beforeEach(() => {
  ...
  serverConnection = createProtocolConnection(
...
describe("Connection Tests", () => {
  ...
  it("should provid token", async () => {
    serverConnection.onRequest(DocumentSymbolRequest.type, (params) => {
      expect(params.partialResultToken).toBe(
        "3b1db4c9-e011-489e-a9d1-0653e64707c2"
      );
      return [];
    });
  ...
  it("should report result", async () => {
    ...
    serverConnection.onRequest(DocumentSymbolRequest.type, (params) => {
      ...
      serverConnection.sendProgress(progressType, params.partialResultToken, [
        result,
      ]);
      return [];
    });
