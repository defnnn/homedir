...
export default class CodeActions {
  constructor(private nvim: Neovim, private handler: HandlerDelegate) {
    ...
    commandManager.titles.set(
      "editor.action.organizeImport",
      "run organize import code action."
    );
  }

  public async codeActionRange(
    start: number,
    end: number,
    only?: string
  ): Promise<void> {
    ...
    let codeActions = await this.getCodeActions(
      doc,
      range,
      only ? [only] : null
    );
    if (!codeActions || codeActions.length == 0) {
      window.showMessage(
        `No${only ? " " + only : ""} code action available`,
        "warning"
      );
      return;
    }
    let idx = await window.showMenuPicker(
  ...
  public async organizeImport(bufnr?: number): Promise<void> {
    ...
    await doc.synchronize();
    let actions = await this.getCodeActions(doc, undefined, [
  ...
  public async getCodeActions(
    ...
    let codeActions = await this.handler.withRequestToken(
      "code action",
      (token) => {
        return languages.getCodeActions(
          doc.textDocument,
          range,
          context,
          token
  ...
  public async doCodeAction(
    ...
    let codeActions = await this.getCodeActions(
      doc,
      range,
      Array.isArray(only) ? only : null
    );
    if (typeof only == "string") {
      codeActions = codeActions.filter(
        (o) => o.title == only || (o.command && o.command.title == only)
      );
    } else if (Array.isArray(only)) {
      codeActions = codeActions.filter((o) =>
        only.some((k) => o.kind && o.kind.startsWith(k))
      );
    }
    if (!codeActions || codeActions.length == 0) {
      window.showMessage(
   ...
   */
  public async getCurrentCodeActions(
    mode?: string,
    only?: CodeActionKind[]
  ): Promise<CodeAction[]> {
    let { doc } = await this.handler.getCurrentState();
  ...
  public async doQuickfix(): Promise<void> {
    let actions = await this.getCurrentCodeActions("line", [
    ...
    await this.applyCodeAction(actions[0]);
    this.nvim.command(
      `silent! call repeat#set("\\<Plug>(coc-fix-current)", -1)`,
      true
    );
  }
  ...
  public async applyCodeAction(action: CodeAction): Promise<void> {
    ...
    action = await this.handler.withRequestToken(
      "resolve codeAction",
      (token) => {
