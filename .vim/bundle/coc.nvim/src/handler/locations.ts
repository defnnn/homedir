...
export default class LocationsHandler {
  ...
  private async request<T>(
    ...
    return await this.handler.withRequestToken(
      method,
      (token) => {
  ...
  public async definitions(): Promise<Location[]> {
    ...
    return languages.getDefinition(
      doc.textDocument,
      position,
      tokenSource.token
    );
  ...
  public async declarations(): Promise<Location | Location[] | LocationLink[]> {
    ...
    await doc.synchronize();
    const tokenSource = new CancellationTokenSource();
    return languages.getDeclaration(
      doc.textDocument,
      position,
      tokenSource.token
    );
  ...
  public async typeDefinitions(): Promise<Location[]> {
    ...
    await doc.synchronize();
    const tokenSource = new CancellationTokenSource();
    return languages.getTypeDefinition(
      doc.textDocument,
      position,
      tokenSource.token
    );
  ...
  public async implementations(): Promise<Location[]> {
    ...
    await doc.synchronize();
    const tokenSource = new CancellationTokenSource();
    return languages.getImplementation(
      doc.textDocument,
      position,
      tokenSource.token
    );
  ...
  public async references(): Promise<Location[]> {
    ...
    await doc.synchronize();
    const tokenSource = new CancellationTokenSource();
    return languages.getReferences(
      doc.textDocument,
  ...
  public async gotoDefinition(openCommand?: string): Promise<boolean> {
    let definition = await this.request(
      "definition",
      (doc, position, token) => {
  ...
  public async getTagList(): Promise<TagDefinition[] | null> {
    ...
    let definitions = await languages.getDefinition(
      doc.textDocument,
      position,
      tokenSource.token
    );
    ...
    return definitions.map((location) => {
      ...
      const filename =
        parsedURI.scheme == "file" ? parsedURI.fsPath : parsedURI.toString();
      return {
        name: word,
        cmd: `keepjumps ${location.range.start.line + 1} | normal ${
  ...
  public async handleLocations(
    definition: Definition | LocationLink[],
    openCommand?: string | false
  ): Promise<void> {
    if (!definition) return;
    let locations: Location[] = Array.isArray(definition)
      ? (definition as Location[])
      : [definition];
    locations = locations.map((o) =>
      LocationLink.is(o) ? Location.create(o.targetUri, o.targetRange) : o
    );
