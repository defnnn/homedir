...
export default class SymbolsOutline {
  ...
  private treeViews: WeakMap<
    BasicDataProvider<OutlineNode>,
    BasicTreeView<OutlineNode>[]
  > = new WeakMap();
  private originalWins: WeakMap<BasicTreeView<OutlineNode>, number> =
    new WeakMap();
  ...
  constructor(
    ...
    events.on(
      ...
      debounce(
        ...
        global.hasOwnProperty("__TEST__") ? 100 : 300
      ),
      null,
      this.disposables
    );
    events.on(
      ...
      async (bufnr) => {
        ...
        let winid = await this.nvim.call("coc#window#find", [
          "cocViewId",
          "OUTLINE",
        ]);
        if (winid == -1) return;
  ...
  private async _onBufEnter(): Promise<void> {
    ...
    let [curr, bufnr, winid] = (await this.nvim.eval(
      `[win_getid(),bufnr('%'),coc#window#find('cocViewId', 'OUTLINE')]`
  ...
  private convertSymbolToNode(
    documentSymbol: DocumentSymbol,
    sortFn: (a: OutlineNode, b: OutlineNode) => number
  ): OutlineNode {
    return {
      label: documentSymbol.name,
      tooltip: documentSymbol.detail,
      description: this.config.showLineNumber
        ? `${documentSymbol.selectionRange.start.line + 1}`
        : undefined,
      ...
      children: Array.isArray(documentSymbol.children)
  ...
  private setMessage(
    provider: BasicDataProvider<OutlineNode>,
  ...
  private createProvider(buf: SymbolsBuffer): BasicDataProvider<OutlineNode> {
    ...
    let convertSymbols = (symbols: DocumentSymbol[]): OutlineNode[] => {
      return symbols
        .map((s) => this.convertSymbolToNode(s, sortFn))
        .sort(sortFn);
    };
    ...
    let provider = new BasicDataProvider({
      ...
      handleClick: async (item) => {
        ...
        buf.highlightRanges("outline-hover", "CocHoverRange", [
          item.selectRange,
        ]);
      ...
      resolveActions: async (_, element) => {
        ...
        if (winnr == -1) return;
        let doc = workspace.getDocument(bufnr);
        let actions = await this.handler.getCodeActions(
          doc,
          element.range,
          this.config.codeActionKinds
        );
  ...
  public async show(keep?: number): Promise<void> {
    ...
    let [bufnr, winid] = (await this.nvim.eval('[bufnr("%"),win_getid()]')) as [
      number,
      number
    ];
