...
export default class Symbols {
  ...
  public async resolveWorkspaceSymbol(
    ...
    let tokenSource = new CancellationTokenSource();
    return await languages.resolveWorkspaceSymbol(
      symbolInfo,
      tokenSource.token
    );
  }
  ...
  public async getCurrentFunctionSymbol(bufnr?: number): Promise<string> {
    ...
    symbols = symbols.filter((s) =>
      ["Class", "Method", "Function", "Struct"].includes(s.kind)
    );
    let functionName = "";
   ...
   */
  public async selectSymbolRange(
    inner: boolean,
    visualmode: string,
    ...
    for (let sym of symbols.reverse()) {
      if (
        sym.range &&
        !equals(sym.range, range) &&
        rangeInRange(range, sym.range)
      ) {
