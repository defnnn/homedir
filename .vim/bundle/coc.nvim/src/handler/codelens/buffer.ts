...
export default class CodeLensBuffer implements BufferSyncItem {
  ...
  private get enabled(): boolean {
    return (
      this.textDocument &&
      this.config.enabled &&
      languages.hasProvider("codeLens", this.textDocument)
    );
  }
  ...
  private async _resolveCodeLenses(): Promise<void> {
    ...
    let [bufnr, start, end] = (await this.nvim.eval(
      `[bufnr('%'),line('w0'),line('w$')]`
    ...
    if (codeLenses.length) {
      ...
      let token = tokenSource.token;
      await Promise.all(
        codeLenses.map((codeLens) => languages.resolveCodeLens(codeLens, token))
      );
  ...
  private setVirtualText(codeLenses: CodeLens[]): void {
    ...
    for (let lnum of list.keys()) {
      ...
      for (let i = 0; i < n_commands; i++) {
        ...
        if (i != n_commands - 1) {
          chunks.push([this.config.subseparator, "CocCodeLens"] as [
            string,
            string
          ]);
        }
      ...
      this.nvim.call(
        ...
        [this.bufnr, this.srcId, lnum, chunks, {}],
        true
      );
    }
  }
