...
interface CallHierarchyProvider
  extends TreeDataProvider<CallHierarchyDataItem> {
  kind: "incoming" | "outgoing";
  dispose: () => void;
}

function isCallHierarchyItem(item: any): item is CallHierarchyItem {
  if (item && item.name && item.kind && Range.is(item.range) && item.uri)
    return true;
  return false;
}
...
export default class CallHierarchyHandler {
    ...
    this.disposables.push(
      commands.registerCommand(
        ...
        async (
          ...
          win.highlightRanges(
            CallHierarchyHandler.rangesHighlight,
            [item.selectionRange],
            10,
            true
          );
  ...
  private createProvider(
    ...
    const findParent = (
      curr: CallHierarchyDataItem,
      element: CallHierarchyDataItem
    ): CallHierarchyDataItem | undefined => {
    ...
    let provider: CallHierarchyProvider = {
      ...
      getTreeItem: (element) => {
        ...
        if (this.config.enableTooltip) {
          item.tooltip = path.relative(
            workspace.cwd,
            URI.parse(element.uri).fsPath
          );
        }
      ...
      getChildren: async (element) => {
        ...
        if (!element) {
          if (!rootItems) {
            rootItems = (await this.prepare(
              doc,
              position,
              token
            )) as CallHierarchyDataItem[];
  ...
  private async getChildren(
    ...
    if (kind == "incoming") {
      ...
      if (res)
        items = res.map((o) =>
          Object.assign(o.to, { ranges: o.fromRanges, sourceUri: item.uri })
        );
    }
    return items;
  }
  ...
  private async getCallHierarchyItems(
    item: CallHierarchyItem | undefined,
    kind: "outgoing"
  ...
  public async showCallHierarchyTree(
    kind: "incoming" | "outgoing"
  ): Promise<void> {
