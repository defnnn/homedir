...
export default class SemanticTokensHighlights {
  ...
  constructor(private nvim: Neovim, private handler: HandlerDelegate) {
    ...
    commands.register(
      {
        ...
        execute: async () => {
          ...
          window.showMessage(
            "Semantic tokens provider found for current buffer",
            "more"
          );
        },
      },
      false,
      "check semantic tokens provider for current buffer"
    );
    commands.register(
      {
      ...
      false,
      "clear semantic tokens highlights of current buffer"
    );
    commands.register(
      {
      ...
      false,
      "clear semantic tokens highlights of all buffers"
    );
    ...
    languages.onDidSemanticTokensRefresh(
      (selector) => {
  ...
  public async showHiglightInfo(): Promise<void> {
    ...
    nvim.command(
      `vs +setl\\ buftype=nofile __coc_semantic_highlights_${item.bufnr}__`,
      true
    );
    nvim.command(
      `setl bufhidden=wipe noswapfile nobuflisted wrap undolevels=-1`,
      true
    );
    ...
    highlighter.addLine("");
    highlighter.addLine(
      "Semantic highlight groups used by current buffer",
      headGroup
    );
    highlighter.addLine("");
