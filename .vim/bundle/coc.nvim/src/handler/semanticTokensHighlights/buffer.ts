...
export default class SemanticTokensBuffer implements SyncItem {
  ...
  public checkState(): void {
    ...
    if (!languages.hasProvider("semanticTokens", doc.textDocument))
      throw new Error(
        "SemanticTokens provider not found, your languageserver may not support it"
      );
  }
  ...
  private async doHighlight(): Promise<void> {
    ...
    if (workspace.env.updateHighlight) {
      prev = (await nvim.call("coc#highlight#get_highlights", [
        this.bufnr,
        NAMESPACE,
      ])) as HighlightItem[];
    }
  ...
  private calculateHighlightUpdates(
    prev: HighlightItem[],
    curr: HighlightItem[]
  ): { highlights: HighlightItem[]; lines: Set<number> } {
  ...
  private async requestHighlights(
    doc: Document,
    forceFull?: boolean
  ): Promise<HighlightItem[] | undefined> {
    ...
    if (hasEditProvider && previousResult?.resultId) {
      result = await languages.provideDocumentSemanticTokensEdits(
        doc.textDocument,
        previousResult.resultId,
        token
      );
    } else {
      result = await languages.provideDocumentSemanticTokens(
        doc.textDocument,
        token
      );
    }
    ...
    for (const { group, deltaLine, deltaStartCharacter, length } of relatives) {
