...
export default class FormatHandler {
  ...
  constructor(private nvim: Neovim, private handler: HandlerDelegate) {
    ...
    handler.addDisposable(
      workspace.onDidChangeConfiguration(this.loadPreferences, this)
    );
    handler.addDisposable(
        ...
        if (filetypes.includes(languageId) || filetypes.includes("*")) {
          let willSaveWaitUntil = async (): Promise<TextEdit[] | undefined> => {
            if (!languages.hasFormatProvider(event.document)) {
              logger.warn(
                `Format provider not found for ${event.document.uri}`
              );
              return undefined;
            }
            ...
            let timer = setTimeout(() => {
              logger.warn(
                `Format on save ${event.document.uri} timeout after 0.5s`
              );
              tokenSource.cancel();
            }, 500);
            let textEdits = await languages.provideDocumentFormattingEdits(
              event.document,
              options,
              tokenSource.token
            );
            ...
            if (!textEdits && !tokenSource.token.isCancellationRequested) {
              logger.want(
                `Format on save ${event.document.uri} get undefined result.`
              );
            }
            return textEdits;
          };
    ...
    handler.addDisposable(
      events.on("CursorMovedI", async (bufnr) => {
  ...
  private loadPreferences(e?: ConfigurationChangeEvent): void {
    if (!e || e.affectsConfiguration("coc.preferences")) {
      ...
      this.preferences = {
        ...
        formatOnSaveFiletypes: config.get<string[]>(
  ...
  private async tryFormatOnType(
    ...
    if (
      filetypes.length &&
      !filetypes.includes(doc.filetype) &&
      !filetypes.includes("*")
    ) {
    ...
    let edits = await this.handler.withRequestToken(
      "Format on type",
      async (token) => {
        ...
        let pos: Position = newLine
          ? { line: position.line - 1, character: origLine.length }
          : position;
        await doc.synchronize();
        return await languages.provideDocumentOnTypeEdits(
          ch,
          doc.textDocument,
          pos,
          token
    ...
    let to = changed
      ? Position.create(
          position.line + changed.line,
          position.character + changed.character
        )
      : null;
  ...
  public async documentFormat(doc: Document): Promise<boolean> {
    ...
    let textEdits = await this.handler.withRequestToken("format", (token) => {
      return languages.provideDocumentFormattingEdits(
        doc.textDocument,
        options,
        token
      );
  ...
  public async documentRangeFormat(
    doc: Document,
    mode?: string
  ): Promise<number> {
    ...
    if (mode) {
      ...
      let [lnum, count, mode] = (await this.nvim.eval(
        "[v:lnum,v:count,mode()]"
    ...
    let options = await workspace.getFormatOptions(doc.uri);
    let textEdits = await this.handler.withRequestToken(
      "Format range",
      (token) => {
        return languages.provideDocumentRangeFormattingEdits(
          doc.textDocument,
          range,
          options,
          token
