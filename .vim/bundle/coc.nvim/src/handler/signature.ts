...
export default class Signature {
  ...
  constructor(private nvim: Neovim, private handler: HandlerDelegate) {
    ...
    events.on(
      "TextInsert",
      async (bufnr, info, character) => {
        if (!this.config.trigger) return;
        let doc = this.getTextDocument(bufnr);
        if (
          !doc ||
          !languages.shouldTriggerSignatureHelp(doc.textDocument, character)
        )
          return;
        await this._triggerSignatureHelp(
          doc,
          { line: info.lnum - 1, character: info.pre.length },
          false
        );
      },
      null,
  ...
  private async _triggerSignatureHelp(
    doc: Document,
    position: Position,
    invoke = true,
    offset = 0
    ...
    let signatureHelp = await languages.getSignatureHelp(
      doc.textDocument,
      position,
      token,
      {
  ...
  private async showSignatureHelp(
    ...
    let content = doc
      .getline(position.line, false)
      .slice(0, position.character);
    this.lastPosition = {
      bufnr: doc.bufnr,
      lnum: position.line + 1,
      col: byteLength(content) + 1,
    };
    const excludeImages = workspace
