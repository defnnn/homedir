...
export default class RefactorBuffer implements BufferSyncItem {
  ...
  private async onDocumentChange(
    ...
    this._fileItems = this._fileItems.filter(
      (o) => o.ranges && o.ranges.length > 0
    );
    if (edits.length) {
      this.changing = true;
  ...
  public async splitOpen(): Promise<void> {
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (ms) {
        ...
        if (err)
          window.showMessage(`Error on open ${filepath}: ${err}`, "error");
        if (!valid) {
    ...
    try {
      ...
      for (let item of items) {
        for (let range of item.ranges) {
          ...
          highligher.addText(
            `${
              isParentFolder(cwd, item.filepath)
                ? path.relative(cwd, item.filepath)
                : item.filepath
            }`
          );
  ...
  private highlightLineNr(): void {
    ...
    if (srcId) {
      ...
      for (let item of fileItems) {
        for (let range of item.ranges) {
          ...
          nvim.call(
            ...
            [bufnr, srcId, range.lnum - 1, [[text, "LineNr"]], {}],
            true
          );
        }
      }
      ...
      if (this.matchIds.size) {
        nvim.call(
          "coc#highlight#clear_matches",
          [winid, Array.from(this.matchIds)],
          true
        );
        this.matchIds.clear();
      }
      ...
      for (let item of fileItems) {
        let filename = `${
          cwd ? path.relative(cwd, item.filepath) : item.filepath
        }`;
