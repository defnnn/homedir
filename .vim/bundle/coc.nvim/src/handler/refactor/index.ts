...
export default class Refactor {
  ...
  public async doRefactor(): Promise<void> {
    ...
    let edit = await this.handler.withRequestToken(
      "refactor",
      async (token) => {
        let res = await languages.prepareRename(
          doc.textDocument,
          position,
          token
        );
        ...
        let edit = await languages.provideRenameEdits(
          doc.textDocument,
          position,
          "NewName",
          token
   ...
   */
  public async createRefactorBuffer(
    filetype?: string
  ): Promise<RefactorBuffer> {
    let { nvim } = this;
    let [fromWinid, cwd] = (await nvim.eval("[win_getid(),getcwd()]")) as [
      number,
      string
    ];
    ...
    nvim.command(
      `setl buftype=acwrite nobuflisted bufhidden=wipe nofen wrap conceallevel=2 concealcursor=n`,
      true
    );
    nvim.command(
      `setl undolevels=-1 nolist nospell noswapfile foldmethod=expr foldexpr=coc#util#refactor_foldlevel(v:lnum)`,
      true
    );
    nvim.command(
      `setl foldtext=coc#util#refactor_fold_text(v:foldstart)`,
      true
    );
    nvim.call(
      "setline",
      [1, ["Save current buffer to make changes", SEPARATOR]],
      true
    );
    ...
    if (err) return;
    let [bufnr, win] = (await nvim.eval('[bufnr("%"),win_getid()]')) as [
      number,
      number
    ];
    let opts = { fromWinid, winid: win, cwd };
    await workspace.document;
    let buf = new RefactorBuffer(
      bufnr,
      this.srcId,
      this.nvim,
      this.config,
  ...
  public async fromLines(lines: string[]): Promise<RefactorBuffer> {
    let buf = await this.createRefactorBuffer();
    if (buf)
      await buf.buffer.setLines(lines, {
        start: 0,
        end: -1,
   ...
   */
  public async fromWorkspaceEdit(
    edit: WorkspaceEdit,
    filetype?: string
  ): Promise<RefactorBuffer> {
