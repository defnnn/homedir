...
export default class Handler implements HandlerDelegate {
  ...
  constructor(private nvim: Neovim) {
    ...
    this.labels = workspace
      .getConfiguration("suggest")
      .get<any>("completionItemKindLabels", {});
  ...
  public async getCurrentState(): Promise<CurrentState> {
    let { nvim } = this;
    let [bufnr, [line, character], winid, mode] = (await nvim.eval(
      "[bufnr('%'),coc#cursor#position(),win_getid(),mode()]"
  ...
  public checkProvier(id: ProviderName, document: TextDocument): void {
    if (!languages.hasProvider(id, document)) {
      throw new Error(
        `${id} provider not found for current buffer, your language server doesn't support it.`
      );
    }
  }

  public async withRequestToken<T>(
    name: string,
  ...
  public getIcon(kind: SymbolKind): { text: string; hlGroup: string } {
    ...
    let defaultIcon =
      typeof labels["default"] === "string"
        ? labels["default"]
        : kindText[0].toLowerCase();
    let text =
      kindText == "Unknown"
        ? ""
        : labels[kindText[0].toLowerCase() + kindText.slice(1)];
    if (!text || typeof text !== "string") text = defaultIcon;
    return {
      text,
      hlGroup:
        kindText == "Unknown" ? "CocSymbolDefault" : `CocSymbol${kindText}`,
    };
  }
