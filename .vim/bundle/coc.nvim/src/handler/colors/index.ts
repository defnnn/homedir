...
export default class Colors {
  ...
  constructor(private nvim: Neovim, private handler: HandlerDelegate) {
    ...
    this.disposables.push(
      commandManager.registerCommand("editor.action.pickColor", () => {
        return this.pickColor();
      })
    );
    commandManager.titles.set(
      "editor.action.pickColor",
      "pick color from system color picker when possible."
    );
    this.disposables.push(
      commandManager.registerCommand("editor.action.colorPresentation", () => {
  ...
  public async pickPresentation(): Promise<void> {
    ...
    if (!info)
      return window.showMessage(
        "Color not found at current position",
        "warning"
      );
    let document = await workspace.document;
    let tokenSource = new CancellationTokenSource();
    let presentations = await languages.provideColorPresentations(
      info,
      document.textDocument,
      tokenSource.token
    );
    if (!presentations?.length) return;
    let res = await window.showMenuPicker(
      presentations.map((o) => o.label),
      "choose color:"
    );
    if (res == -1) return;
  ...
  public async getColorInformation(
    bufnr: number
