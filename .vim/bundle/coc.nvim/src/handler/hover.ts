...
export default class HoverHandler {
  ...
  private registerProvider(): void {
    ...
    let provider: TextDocumentContentProvider = {
      ...
      provideTextDocumentContent: async () => {
        ...
        nvim.command(
          `if winnr('j') != winnr('k') | exe "normal! z${Math.min(
            this.documentLines.length,
    ...
    this.disposables.push(
      workspace.registerTextDocumentContentProvider("coc", provider)
    );
  }
  ...
  private loadConfiguration(e?: ConfigurationChangeEvent): void {
    if (!e || e.affectsConfiguration("hover")) {
      ...
      this.config = {
        ...
        target:
          target == "float" && !workspace.floatSupported ? "preview" : target,
      ...
      this.excludeImages = preferences.get<boolean>(
  ...
  public async onHover(hoverTarget?: HoverTarget): Promise<boolean> {
    ...
    let hovers = await this.handler.withRequestToken(
      "hover",
      (token) => {
  ...
  public async definitionHover(hoverTarget: HoverTarget): Promise<boolean> {
    ...
    this.handler.checkProvier("hover", doc.textDocument);
    await doc.synchronize();
    const hovers: (Hover | Documentation)[] =
      await this.handler.withRequestToken(
        "hover",
        (token) => {
          return languages.getHover(doc.textDocument, position, token);
        },
    ...
    const defs = await this.handler.withRequestToken(
      "definitionHover",
      (token) => {
    ...
    if (defs?.length) {
      for (const def of defs) {
        ...
        if (lines.length) {
          ...
          if (indent)
            lines = lines.map((l) =>
              l.startsWith(indent) ? l.substring(indent.length) : l
            );
  ...
  private async previewHover(
    hovers: (Hover | Documentation)[],
    target?: string
  ): Promise<void> {
    ...
    for (let hover of hovers) {
      ...
      if (Array.isArray(contents)) {
        ...
        addDocument(
          docs,
          contents.value,
          isMarkdown(contents) ? "markdown" : "txt",
          isPreview
        );
      }
    }
  ...
  public async getHover(): Promise<string[]> {
    ...
    let hovers = await languages.getHover(
      doc.textDocument,
      position,
      tokenSource.token
    );
