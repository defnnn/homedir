...
class Window {
   ...
   */
  public async runTerminalCommand(
    cmd: string,
    cwd?: string,
    keepfocus = false
  ): Promise<TerminalResult> {
    cwd = cwd || workspace.cwd;
    return (await this.nvim.callAsync("coc#util#run_terminal", {
      cmd,
      cwd,
      keepfocus: keepfocus ? 1 : 0,
    })) as TerminalResult;
  }
  ...
  public async openTerminal(
    ...
    let bufnr = await this.nvim.call("coc#util#open_terminal", {
      cmd,
      ...opts,
    });
  ...
  public async showQuickpick(
    ...
    try {
      ...
      let res = await this.nvim.callAsync("coc#util#quickpick", [
        title,
        items.map((s) => s.trim()),
      ]);
      release();
  ...
  public async showMenuPicker(
    ...
    if (workspace.env.dialog) {
      ...
      try {
        let menu = new Menu(
          this.nvim,
          { items: items.map((s) => s.trim()), title },
          token
        );
  ...
  public async openLocalConfig(): Promise<void> {
    ...
    if (root == os.homedir()) {
      this.showMessage(
        `Can't create local config in home directory`,
        "warning"
      );
      return;
    }
    ...
    if (!fs.existsSync(dir)) {
      let res = await this.showPrompt(
        `Would you like to create folder'${root}/.vim'?`
      );
      if (!res) return;
      fs.mkdirSync(dir);
    }
    await workspace.jumpTo(
      URI.file(path.join(dir, CONFIG_FILE_NAME)).toString()
    );
  }
  ...
  public async requestInput(
    ...
    if (
      workspace.env.dialog &&
      preferences.get<boolean>("promptInput", true) &&
      !isWindows
    ) {
      let release = await this.mutex.acquire();
      ...
      try {
        ...
        let arr = (await nvim.call("coc#float#create_prompt_win", [
          title,
          defaultValue || "",
          opts,
        ])) as [number, number];
      ...
      let res = await workspace.callAsync<string>("input", [
        title + ": ",
        defaultValue || "",
      ]);
  ...
  public createStatusBarItem(
    ...
    return this.statusLine.createStatusBarItem(
      priority,
  ...
  public async echoLines(lines: string[], truncate = false): Promise<void> {
    ...
    if (truncate && lines.length == cmdHeight) {
      ...
      lines[cmdHeight - 1] = `${
        last.length == maxLen ? last.slice(0, -4) : last
      } ...`;
    }
  ...
  public async getCursorPosition(): Promise<Position> {
    ...
    let [line, content] = (await this.nvim.eval(
      `[line('.')-1, strpart(getline('.'), 0, col('.') - 1)]`
    )) as [number, string];
  ...
  public async moveTo(position: Position): Promise<void> {
    await this.nvim.call("coc#cursor#move_to", [
      position.line,
      position.character,
    ]);
  ...
  public async getCursorScreenPosition(): Promise<ScreenPosition> {
    let [row, col] = (await this.nvim.call("coc#cursor#screen_pos")) as [
      number,
      number
    ];
    return { row, col };
   ...
   */
  public async showPickerDialog(
    items: string[],
    title: string,
    token?: CancellationToken
  ): Promise<string[] | undefined>;
  public async showPickerDialog<T extends QuickPickItem>(
    items: T[],
    title: string,
    token?: CancellationToken
  ): Promise<T[] | undefined>;
  public async showPickerDialog(
    ...
    try {
      ...
      let res =
        picked == undefined
          ? undefined
          : items.filter((_, i) => picked.includes(i));
      release();
      return res;
    } catch (e) {
  ...
  public async showInformationMessage<T extends MessageItem>(
    message: string,
    ...items: T[]
  ): Promise<T | undefined>;
  public async showInformationMessage<T extends MessageItem | string>(
    message: string,
    ...items: T[]
  ): Promise<T | undefined> {
    if (!this.enableMessageDialog)
      return (await this.showConfirm(message, items, "Info")) as any;
    let texts =
      typeof items[0] === "string"
        ? items
        : (items as any[]).map((s) => s.title);
  ...
  public async showWarningMessage<T extends MessageItem | string>(
    message: string,
    ...items: T[]
  ): Promise<T | undefined> {
    if (!this.enableMessageDialog)
      return (await this.showConfirm(message, items, "Warning")) as any;
    let texts =
      typeof items[0] === "string"
        ? items
        : (items as any[]).map((s) => s.title);
  ...
  public async showErrorMessage<T extends MessageItem>(
    message: string,
    ...items: T[]
  ): Promise<T | undefined>;
  public async showErrorMessage<T extends MessageItem | string>(
    message: string,
    ...items: T[]
  ): Promise<T | undefined> {
    if (!this.enableMessageDialog)
      return (await this.showConfirm(message, items, "Error")) as any;
    let texts =
      typeof items[0] === "string"
        ? items
        : (items as any[]).map((s) => s.title);
  ...
  private async showConfirm<T extends MessageItem | string>(
    message: string,
    items: T[],
    kind: "Info" | "Warning" | "Error"
  ): Promise<T> {
    if (!items || items.length == 0) {
      let msgType: MsgTypes =
        kind == "Info" ? "more" : kind == "Error" ? "error" : "warning";
      this.showMessage(message, msgType);
      return undefined;
    }
    let titles: string[] =
      typeof items[0] === "string"
        ? (items.slice() as string[])
        : items.map((o) => (o as MessageItem).title);
    let choices = titles.map((s, i) => `${i + 1}${s}`);
    let res = await this.nvim.callAsync("coc#util#with_callback", [
      "confirm",
  ...
  private createNotification(
    ...
    return new Promise((resolve) => {
      ...
      let notification = new Notification(this.nvim, config);
      notification.show(this.notificationPreference).then(
        (shown) => {
          if (!shown) {
  ...
  private checkDialog(): boolean {
    ...
    this.showMessage(
      "Dialog requires vim >= 8.2.0750 or neovim >= 0.4.0, please upgrade your vim",
      "warning"
    );
    return false;
  }
