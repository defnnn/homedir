...
export class Workspace implements IWorkspace {
  ...
  private schemeProviderMap: Map<string, TextDocumentContentProvider> =
    new Map();
  ...
  public readonly onDidOpenTextDocument: Event<
    TextDocument & { bufnr: number }
  > = this._onDidOpenDocument.event;
  public readonly onDidCloseTextDocument: Event<
    TextDocument & { bufnr: number }
  > = this._onDidCloseDocument.event;
  ...
  public readonly onWillSaveTextDocument: Event<TextDocumentWillSaveEvent> =
    this._onWillSaveDocument.event;
  public readonly onDidSaveTextDocument: Event<TextDocument> =
  ...
  public readonly onDidWorkspaceInitialized: Event<void> =
    this._onDidWorkspaceInitialized.event;
  public readonly onDidRuntimePathChange: Event<string[]> =
    this._onDidRuntimePathChange.event;
  ...
  public async init(): Promise<void> {
    ...
    if (this._env.apiversion != APIVERSION) {
      console.error(
        `API version ${this._env.apiversion} is not ${APIVERSION}, please build coc.nvim by 'yarn install' after pull source code.`
      );
      process.exit();
    }
    this._insertMode = this._env.mode.startsWith("insert");
    if (
      this._env.workspaceFolders &&
      Array.isArray(this._env.workspaceFolders)
    ) {
  ...
  public watchOption(
    ...
    let disposable = events.on(
      "OptionSet",
      async (changed: string, oldValue: any, newValue: any) => {
  ...
  public async resolveRootFolder(
    uri: URI,
    patterns: string[]
  ): Promise<string> {
    let { cwd } = this;
  ...
  public createFileSystemWatcher(
    ...
    let watchmanPath = global.hasOwnProperty("__TEST__")
      ? null
      : this.getWatchmanPath();
    let channel: OutputChannel = watchmanPath
      ? window.createOutputChannel("watchman")
      : null;
    let promise = watchmanPath
      ? Watchman.createClient(watchmanPath, this.root, channel)
  ...
  public async applyEdit(edit: WorkspaceEdit): Promise<boolean> {
    ...
    let [bufnr, cursor] = (await nvim.eval(
      '[bufnr("%"),coc#cursor#position()]'
    ...
    let promptUser =
      ...
      preferences.get<boolean>("promptWorkspaceEdit", true);
    ...
    try {
      if (documentChanges && documentChanges.length) {
        ...
        if (promptUser) {
          ...
          if (diskCount) {
            let res = await window.showPrompt(
              `${diskCount} documents on disk would be loaded for change, confirm?`
            );
            if (!res) return;
          }
        }
        ...
        for (const change of documentChanges) {
          if (TextDocumentEdit.is(change)) {
            ...
            await this.renameFile(
              URI.parse(change.oldUri).fsPath,
              URI.parse(change.newUri).fsPath,
              change.options
            );
        ...
        if (unloaded.length) {
          if (promptUser) {
            let res = await window.showPrompt(
              `${unloaded.length} documents on disk would be loaded for change, confirm?`
            );
            if (!res) return;
          }
      ...
      if (currEdits) {
        let changed = getChangedFromEdits(
      ...
      if (locations.length) {
        let items = await Promise.all(
          locations.map((loc) => this.getQuickfixItem(loc))
        );
        ...
        if (listTarget == "quickfix") {
          ...
          if (!silent)
            window.showMessage(
              `changed ${changeCount} buffers, use :wa to save changes to disk and :copen to open quickfix list`,
              "more"
            );
          ...
          if (!silent)
            window.showMessage(
              `changed ${changeCount} buffers, use :wa to save changes to disk and :CocList location to manage changed locations`,
              "more"
            );
        }
      }
  ...
  public async getSelectedRange(
    ...
    if (mode === "cursor") {
      let [line, character] = (await nvim.eval("coc#cursor#position()")) as [
        number,
        number
      ];
    ...
    let [, sl, sc] = (await nvim.call("getpos", isVisual ? `'<` : `'[`)) as [
      number,
      number,
      number
    ];
    let [, el, ec] = (await nvim.call("getpos", isVisual ? `'>` : `']`)) as [
      number,
      number,
      number
    ];
    let range = Range.create(
      document.getPosition(sl, sc),
      document.getPosition(el, ec)
    );
    if (mode == "v" || mode == "\x16") {
  ...
  public async selectRange(range: Range): Promise<void> {
    let { nvim } = this;
    let { start, end } = range;
    let [bufnr, ve, selection] = (await nvim.eval(
      `[bufnr('%'), &virtualedit, &selection]`
    ...
    move_cmd += "v";
    endCol = (await nvim.eval(
      `virtcol([${end.line + 1}, ${endCol}])`
    )) as number;
    ...
    nvim.command(
      `noa call cursor(${start.line + 1},${col + (move_cmd == "a" ? 0 : 1)})`,
      true
    );
  ...
  public async showLocations(locations: Location[]): Promise<void> {
    ...
    if (preferences.get<boolean>("useQuickfixForLocations", false)) {
      let openCommand = (await nvim.getVar(
  ...
  public getWorkspaceFolder(uri: string): WorkspaceFolder | null {
    ...
    return this.workspaceFolders.find((folder) =>
      isParentFolder(URI.parse(folder.uri).fsPath, filepath, true)
    );
  }
  ...
  public async getFormatOptions(uri?: string): Promise<FormattingOptions> {
    ...
    let [tabSize, insertSpaces] = (await this.nvim.call(
  ...
  public async jumpTo(
    ...
    const preferences = this.getConfiguration("coc.preferences");
    let jumpCommand =
      openCommand || preferences.get<string>("jumpCommand", "edit");
    let { nvim } = this;
  ...
  public async loadFiles(uris: string[]): Promise<void> {
    ...
    let bufnrs = (await this.nvim.call("coc#util#open_files", [
      uris.map((u) => URI.parse(u).fsPath),
    ])) as number[];
        ...
        async (uri) => {
          ...
          if (doc) {
            ...
            let content = await Promise.resolve(
              provider.provideTextDocumentContent(uri, tokenSource.token)
            );
  ...
  public registerKeymap(
    ...
    opts = Object.assign(
      { sync: true, cancel: true, silent: true, repeat: false },
      opts
    );
    let { nvim } = this;
    ...
    for (let m of modes) {
      if (m == "i") {
        nvim.command(
          `inoremap ${silent}<expr> <Plug>(coc-${key}) coc#_insert_key('${method}', '${key}', ${
            opts.cancel ? 1 : 0
          })`,
          true
        );
        ...
        nvim.command(
          `${m}noremap ${silent} <Plug>(coc-${key}) :${modify}call coc#rpc#${method}('doKeymap', ['${key}'])<cr>`,
          true
        );
      }
    }
  ...
  public registerExprKeymap(
    ...
    if (!key) return;
    let id = `${mode}${global.Buffer.from(key).toString("base64")}${
      buffer ? "1" : "0"
    }`;
    let { nvim } = this;
    ...
    if (mode == "i") {
      nvim.command(
        `inoremap <silent><expr>${
          buffer ? "<nowait><buffer>" : ""
        } ${key} coc#_insert_key('request', '${id}')`,
        true
      );
    } else {
      nvim.command(
        `${mode}noremap <silent><expr>${
          buffer ? "<nowait><buffer>" : ""
        } ${key} coc#rpc#request('doKeymap', ['${id}'])`,
        true
      );
    }
    return Disposable.create(() => {
    ...
    if (this.isNvim && !global.hasOwnProperty("__TEST__")) {
      nvim.call(
        ...
        [
          ...
          mode,
          key,
          `:${modify}call coc#rpc#${method}('doKeymap', ['${id}', '', '${escaped}'])<CR>`,
          {
            silent: true,
            nowait: true,
          },
  ...
  public registerBufferSync<T extends SyncItem>(
  ...
  public setupDynamicAutocmd(initialize = false): void {
    ...
    for (let scheme of schemes) {
      cmds.push(
        `autocmd BufReadCmd,FileReadCmd,SourceCmd ${scheme}:/* call coc#rpc#request('CocAutocmd', ['BufReadCmd','${scheme}', expand('<amatch>')])`
      );
    }
    for (let [id, autocmd] of this.autocmds.entries()) {
      let args =
        autocmd.arglist && autocmd.arglist.length
          ? ", " + autocmd.arglist.join(", ")
          : "";
      let event = Array.isArray(autocmd.event)
        ? autocmd.event.join(",")
        : autocmd.event;
      ...
      cmds.push(
        `autocmd ${event} ${pattern} call coc#rpc#${
          autocmd.request ? "request" : "notify"
        }('doAutocmd', [${id}${args}])`
      );
    }
    for (let key of this.watchedOptions) {
      cmds.push(
        `autocmd OptionSet ${key} call coc#rpc#notify('OptionSet',[expand('<amatch>'), v:option_old, v:option_new])`
      );
    }
    let content = `
...
augroup end`;
    try {
      ...
      if (this.env.isCygwin && platform.isWindows) {
        cmd = `execute "source" . substitute(system('cygpath ${filepath.replace(
          /\\/g,
          "/"
        )}'), '\\n', '', 'g')`;
      }
  ...
  private async onBufReadCmd(scheme: string, uri: string): Promise<void> {
    ...
    let tokenSource = new CancellationTokenSource();
    let content = await Promise.resolve(
      provider.provideTextDocumentContent(URI.parse(uri), tokenSource.token)
    );
  ...
  public async attach(): Promise<void> {
    ...
    this._attached = true;
    let [bufs, bufnr, winid] = (await this.nvim.eval(
  ...
  private getChangedUris(documentChanges: DocumentChange[] | null): string[] {
    ...
    for (let change of documentChanges) {
      if (TextDocumentEdit.is(change)) {
        ...
        throw new Error(
          `Invalid document change: ${JSON.stringify(change, null, 2)}`
        );
      }
    }
  ...
  private createConfigurations(): Configurations {
    let home =
      path.normalize(process.env.COC_VIMCONFIG) ||
  ...
  private async onBufCreate(buf: number | Buffer): Promise<void> {
    ...
    if (document.enabled) {
      let textDocument: TextDocument & { bufnr: number } = Object.assign(
  ...
  private async onBufWritePre(bufnr: number): Promise<void> {
    ...
    let event: TextDocumentWillSaveEvent = {
      ...
      waitUntil: (thenable: Thenable<any>) => {
        if (!firing) {
          ...
          window.showMessage(
            `waitUntil can't be used in async manner, check log for details`,
            "error"
          );
        } else {
    ...
    if (total) {
      let promise = new Promise<TextEdit[] | undefined>((resolve) => {
        const preferences = this.getConfiguration("coc.preferences");
        const willSaveHandlerTimeout = preferences.get<number>(
          "willSaveHandlerTimeout",
          500
        );
        let timer = setTimeout(() => {
          window.showMessage(
            `Will save handler timeout after ${willSaveHandlerTimeout}ms`,
            "warning"
          );
          resolve(undefined);
        }, willSaveHandlerTimeout);
  ...
  private resolveRoot(document: Document): string {
    ...
    if (this.cwd != os.homedir() && isParentFolder(this.cwd, dir, true))
      return this.cwd;
    return null;
  }

  public getRootPatterns(
    document: Document,
    ...
    return preferences
      .get<string[]>("rootPatterns", [".git", ".hg", ".projections.json"])
      .slice();
  }
  ...
  public async renameCurrent(): Promise<void> {
    ...
    let newPath = await nvim.callAsync("coc#util#with_callback", [
      "input",
      ["New path: ", oldPath, "file"],
    ]);
    ...
    if (exists) {
      ...
      if (
        ...
        let overwrite = await window.showPrompt(
    ...
    let filepath = isParentFolder(cwd, newPath)
  ...
  public removeWorkspaceFolder(fsPath: string): void {
    let idx = this._workspaceFolders.findIndex(
      (f) => URI.parse(f.uri).fsPath == fsPath
    );
    if (idx != -1) {
  ...
  public renameWorkspaceFolder(oldPath: string, newPath: string): void {
    let idx = this._workspaceFolders.findIndex(
      (f) => URI.parse(f.uri).fsPath == oldPath
    );
    if (idx == -1) return;
  ...
  private getServerRootPatterns(filetype: string): string[] {
    let lspConfig = this.getConfiguration().get<{ [key: string]: unknown }>(
      "languageserver",
      {}
    );
    let patterns: string[] = [];
