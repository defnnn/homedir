...
export default class ListSession {
  ...
  constructor(
    ...
    this.worker = new Worker(nvim, list, prompt, listOptions, {
      interactiveDebounceTime: config.get<number>(
        "interactiveDebounceTime",
        100
      ),
      extendedSearchMode: config.get<boolean>("extendedSearchMode", true),
    });
    this.interactiveDebounceTime = config.get<number>(
      "interactiveDebounceTime",
      100
    );
    let debouncedChangeLine = debounce(async () => {
      let [previewing, currwin, lnum] = (await nvim.eval(
        '[coc#list#has_preview(),win_getid(),line(".")]'
    ...
    let debounced = debounce(async () => {
      ...
      if (!autoPreview) {
        let [previewing, mode] = (await nvim.eval(
          "[coc#list#has_preview(),mode()]"
        )) as [number, string];
    ...
    this.worker.onDidChangeItems(
      async ({ items, reload, append, finished }) => {
    ...
    this.worker.onDidChangeLoading(
      (loading) => {
  ...
  public async chooseAction(): Promise<void> {
    ...
    if (invalids.length) {
      nvim.echoError(
        `Can't create shortcut for actions: ${invalids.join(",")} of "${
          this.name
        }" list`
      );
    }
  ...
  public async showHelp(): Promise<void> {
    ...
    nvim.command(
      "setl nobuflisted noswapfile buftype=nofile bufhidden=wipe",
      true
    );
    ...
    highligher.addLine(
      `  :CocList [LIST OPTIONS] ${list.name}${
    ...
    if (Object.keys(config).length) {
      ...
      for (let key of Object.keys(config)) {
        ...
        let description =
          props[name] && props[name].description
    ...
    highligher.addLine("");
    highligher.addLine(
      `see ':h coc-list-options' for available list options.`,
      "Comment"
    );
    nvim.pauseNotification();
  ...
  private async doItemAction(
    ...
    try {
      ...
      if (action.multiple) {
        ...
        await Promise.all(
          items.map((item) =>
            Promise.resolve(action.execute(item, this.context))
          )
        );
      } else {
  ...
  public dispose(): void {
    if (!this.hidden) {
      ...
      if (this.window && winid) {
        this.nvim.call(
          "coc#list#hide",
          [this.window.id, this.savedHeight, winid],
          true
        );
      }
    }
