...
export default class Worker {
  ...
  public readonly onDidChangeItems: Event<ListItemsEvent> =
    this._onDidChangeItems.event;
  public readonly onDidChangeLoading: Event<boolean> =
    this._onDidChangeLoading.event;
    ...
    if (matcher == "strict") {
      ...
      for (let item of items) {
        ...
        for (let input of inputs) {
          let idx = ignorecase
            ? filterLabel.toLowerCase().indexOf(input.toLowerCase())
            : filterLabel.indexOf(input);
          if (idx == -1) {
            match = false;
            break;
          }
          spans.push([
            byteIndex(filterLabel, idx),
            byteIndex(filterLabel, idx + byteLength(input)),
          ]);
        }
        if (match) {
          filtered.push(
    ...
    if (matcher == "regex") {
      ...
      for (let item of items) {
        ...
        for (let regex of regexes) {
          ...
          spans.push([
            byteIndex(filterLabel, ms.index),
            byteIndex(filterLabel, ms.index + byteLength(ms[0])),
          ]);
        }
        if (match) {
          filtered.push(
...
function getFilterLabel(item: ListItem): string {
  return item.filterText != null
    ? patchLine(item.filterText, item.label)
    : item.label;
}
