...
export default abstract class BasicList implements IList, Disposable {
  ...
  public addLocationActions(): void {
    ...
    this.createAction({
      ...
      execute: async (items: ListItem[]) => {
        let quickfixItems = await Promise.all(
          items.map((item) =>
            this.convertLocation(item.location).then((loc) =>
              workspace.getQuickfixItem(loc)
            )
          )
        );
  ...
  public async convertLocation(
    ...
    if (line != null) {
      ...
      let end = Position.create(
        n,
        character + (location.text ? location.text.length : 0)
      );
      return Location.create(
        location.uri,
        Range.create(Position.create(n, character), end)
      );
    }
    return Location.create(location.uri, Range.create(0, 0, 0, 0));
  }
  ...
  protected async previewLocation(
    location: Location,
    context: ListContext
  ): Promise<void> {
