...
export default class ListUI {
  ...
  constructor(
    ...
    this.matchHighlightGroup = config.get<string>(
      "matchHighlightGroup",
      "Search"
    );
    ...
    let debounced = debounce(async (bufnr) => {
      if (bufnr != this.bufnr) return;
      let [winid, start, end] = (await nvim.eval(
        '[win_getid(),line("w0"),line("w$")]'
      )) as number[];
  ...
  public async onMouse(event: MouseEvent): Promise<void> {
    ...
    if (!window) return;
    let [winid, lnum, col] = (await nvim.eval(
  ...
  public async resume(): Promise<void> {
    ...
    for (let lnum of selected) {
      this.buffer?.placeSign({
        lnum,
        id: this.signOffset + lnum,
        name: "CocSelected",
        group: "coc-list",
      });
    }
  ...
  public async drawItems(
    ...
    this.items =
      items.length > this.limitLines ? items.slice(0, this.limitLines) : items;
    if (!this.window) {
      let { position, numberSelect } = listOptions;
      let [bufnr, winid] = await nvim.call("coc#list#create", [
        position,
        height,
        name,
        numberSelect,
  ...
  private setLines(lines: string[], append = false, index: number): void {
    ...
    if (!append) {
      ...
      if (!lines.length) {
        ...
        nvim.call(
          "coc#compat#matchaddpos",
          ["Comment", [[1]], 99, window.id],
          true
        );
      }
    }
    buffer.setOption("modifiable", true, true);
    void buffer.setLines(
      lines,
      { start: append ? -1 : 0, end: -1, strictIndexing: false },
      true
    );
    ...
    if (!append && index == 0) {
      ...
      this.doHighlight(
        Math.max(0, index - height),
        Math.min(index + height + 1, this.length - 1)
      );
    }
    if (!append) {
      this.currIndex = index;
      ...
      if (ansiHighlights) {
        for (let hi of ansiHighlights) {
          ...
          groups.push({
            hlGroup,
            priority: 9,
            pos: [i + 1, span[0] + 1, span[1] - span[0]],
          });
        }
      }
