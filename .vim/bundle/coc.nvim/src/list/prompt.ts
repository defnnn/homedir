...
export default class Prompt {
  ...
  public readonly onDidChangeInput: Event<string> =
    this._onDidChangeInput.event;
  ...
  public drawPrompt(): void {
    ...
    if (this.mode == "insert") {
      if (interactive) {
        ...
        cmds.push(
          `echohl MoreMsg | echon '${_matcher.toUpperCase()} ' | echohl None`
        );
      }
      ...
      if (cusorIndex == input.length) {
        ...
        cmds.push(
          `echohl Cursor | echon '${input[cusorIndex].replace(
            /'/,
            "''"
          )}' | echohl None`
        );
