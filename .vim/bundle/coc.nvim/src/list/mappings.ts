...
export default class Mappings {
  ...
  constructor(
    ...
    this.add("insert", "<PageDown>", () =>
      manager.feedkeys("<PageDown>", false)
    );
    ...
    this.add(
      "insert",
      ["<ScrollWheelUp>"],
      this.doScroll.bind(this, "<ScrollWheelUp>")
    );
    this.add(
      "insert",
      ["<ScrollWheelDown>"],
      this.doScroll.bind(this, "<ScrollWheelDown>")
    );
  ...
  private fixUserMappings(mappings: {
    [key: string]: string;
  }): Map<string, string> {
    ...
    for (let [key, value] of Object.entries(mappings)) {
      if (key.length == 1) {
        ...
        if (key.toLowerCase() == "<space>") {
          ...
          if (!find)
            window.showMessage(
              `Invalid list mappings key configuration: "${key}"`,
              "warning"
            );
        }
      } else {
