...
export default class LocationList extends BasicList {
  ...
  public async loadItems(
    ...
    locs.forEach((loc) => {
      if (!loc.uri) {
        let fullpath = path.isAbsolute(loc.filename)
          ? loc.filename
          : path.join(context.cwd, loc.filename);
    ...
    let ignoreFilepath = locs.every(
      (o) => o.bufnr && bufnr && o.bufnr == bufnr
    );
    let items: ListItem[] = locs.map((loc) => {
      let filename = ignoreFilepath ? "" : loc.filename;
      let filterText = `${filename}${loc.text.trim()}`;
      if (path.isAbsolute(filename)) {
        filename = isParentFolder(context.cwd, filename)
          ? path.relative(context.cwd, filename)
          : filename;
      }
      let pre = `${filename} |${loc.type ? loc.type + " " : ""}${
        loc.lnum
      } col ${loc.col}| `;
      ...
      if (loc.range && loc.range.start.line == loc.range.end.line) {
        let start =
          byteLength(pre) +
          byteLength(loc.text.slice(0, loc.range.start.character));
        let end =
          byteLength(pre) +
          byteLength(loc.text.slice(0, loc.range.end.character));
  ...
  public doHighlight(): void {
    ...
    nvim.command(
      "syntax match CocLocationName /\\v^[^|]+/ contained containedin=CocLocationLine",
      true
    );
    nvim.command(
      "syntax match CocLocationPosition /\\v\\|\\w*\\s?\\d+\\scol\\s\\d+\\|/ contained containedin=CocLocationLine",
      true
    );
    nvim.command(
      "syntax match CocLocationError /Error/ contained containedin=CocLocationPosition",
      true
    );
    nvim.command(
      "syntax match CocLocationWarning /Warning/ contained containedin=CocLocationPosition",
      true
    );
