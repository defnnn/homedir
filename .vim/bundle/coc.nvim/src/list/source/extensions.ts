...
export default class ExtensionList extends BasicList {
  ...
  constructor(nvim: Neovim) {
    ...
    this.addAction("configuration", async (item) => {
      ...
      if (fs.existsSync(jsonFile)) {
        ...
        await workspace.jumpTo(URI.file(jsonFile).toString(), {
          line: idx == -1 ? 0 : idx,
          character: 0,
        });
      }
    ...
    this.addAction("help", async (item) => {
      ...
      if (file)
        await workspace.callAsync("coc#util#jump", [
          "edit",
          path.join(root, file),
        ]);
    });
    ...
    this.addAction("fix", async (item) => {
      ...
      terminal.sendText(
        `${npm} install --production --ignore-scripts --no-lockfile`,
        true
      );
    });
  ...
  public doHighlight(): void {
    ...
    nvim.command(
      "syntax match CocExtensionsActivited /\\v^\\*/ contained containedin=CocExtensionsLine",
      true
    );
    nvim.command(
