...
export class ListManager implements Disposable {
  ...
  public init(nvim: Neovim): void {
    ...
    nvim.command(
      `sign define CocSelected text=${signText} texthl=CocSelectedText linehl=CocSelectedLine`,
      true
    );
  ...
  public parseArgs(
    args: string[]
  ): { list: IList; options: ListOptions; listArgs: string[] } | null {
    let options: string[] = [];
    ...
    if (!listOptions.length && !listArgs.length)
      listOptions = config.get<string[]>("defaultOptions", []);
    ...
    if (interactive && !list.interactive) {
      window.showMessage(
        `Interactive mode of "${name}" list not supported`,
        "error"
      );
      return null;
    }
