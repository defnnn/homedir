...
export class Completion implements Disposable {
  ...
  public init(): void {
    ...
    workspace.watchOption(
      ...
      async (_, newValue) => {
        ...
        if (this.config.autoTrigger === "always") {
          let content = (await this.nvim.call("execute", [
            "verbose set completeopt",
          ])) as string;
          let lines = content.split(/\r?\n/);
          console.error(
            `Some plugin change completeopt during completion: ${lines[
              lines.length - 1
            ].trim()}!`
          );
        }
    ...
    this.excludeImages = workspace
      .getConfiguration("coc.preferences")
      .get<boolean>("excludeImageLinksInMarkdownDocument");
  ...
  private getCompleteConfig(): CompleteConfig {
    ...
    if (keepCompleteopt && autoTrigger != "none") {
      ...
      if (
        !completeOpt.includes("noinsert") &&
        !completeOpt.includes("noselect")
      ) {
        autoTrigger = "none";
      }
    }
    let floatEnable =
      workspace.floatSupported && getConfig<boolean>("floatEnable", true);
    let acceptSuggestionOnCommitCharacter =
      workspace.env.pumevent &&
      getConfig<boolean>("acceptSuggestionOnCommitCharacter", false);
    return {
      autoTrigger,
      floatEnable,
      keepCompleteopt,
      ...
      previewIsKeyword: getConfig<string>(
        "previewIsKeyword",
        "@,48-57,_192-255"
      ),
      ...
      triggerAfterInsertEnter: getConfig<boolean>(
        "triggerAfterInsertEnter",
        false
      ),
      ...
      highPrioritySourceLimit: getConfig<number>(
  ...
  private async showCompletion(
    ...
    let preselect = this.config.enablePreselect
      ? items.findIndex((o) => o.preselect)
      : -1;
  ...
  private async _doComplete(option: CompleteOption): Promise<void> {
    ...
    complete.onDidComplete(async () => {
      ...
      if (search == input) {
        let items = complete.filterResults(
          search,
  ...
  private async onTextChangedP(
    ...
    if (
      !option ||
      option.bufnr != bufnr ||
      info.changedtick == this.changedTick
    )
      return;
  ...
  private async onTextChangedI(
    ...
    if (!this.activated) {
      ...
      let triggerSources = sources.getTriggerSources(
        pretext,
        doc.filetype,
        doc.uri
      );
    ...
    if (
      ...
      if (sources.shouldCommit(resolvedItem, last)) {
        ...
        let newLine = `${line.slice(
          0,
          col
        )}${word}${latestInsertChar}${line.slice(colnr - 1)}`;
  ...
  private async triggerCompletion(doc: Document, pre: string): Promise<void> {
    ...
    await doc.patchChange();
    let [disabled, option] = (await this.nvim.eval(
      '[get(b:,"coc_suggest_disable",0),coc#util#get_complete_option()]'
    )) as [number, CompleteOption];
    if (disabled == 1) {
      logger.warn(`Suggest disabled by b:coc_suggest_disable`);
      return;
    }
  ...
  private async onCompleteDone(item: VimCompleteItem): Promise<void> {
    ...
    if (this.insertCharTs != timestamp || this.insertLeaveTs != insertLeaveTs)
      return;
    let [visible, lnum, pre] = (await this.nvim.eval(
      `[pumvisible(),line('.'),strpart(getline('.'), 0, col('.') - 1)]`
    )) as [number, number, string];
    if (
      visible ||
      lnum != opt.linenr ||
      this.activated ||
      !pre.endsWith(resolvedItem.word)
    )
      return;
    await document.patchChange(true);
  ...
  private async doCompleteResolve(
    item: ExtendedCompleteItem,
  ...
  private async onInsertEnter(bufnr: number): Promise<void> {
    if (
      !this.config.triggerAfterInsertEnter ||
      this.config.autoTrigger !== "always"
    )
      return;
    let doc = workspace.getDocument(bufnr);
    if (!doc) return;
    let pre = (await this.nvim.eval(
  ...
  private async onPumChange(): Promise<void> {
    ...
    if (!docs && resolvedItem.info) {
      ...
      docs = [
        { filetype: isText ? "txt" : this.document.filetype, content: info },
      ];
    }
    if (!docs || docs.length == 0) {
      ...
      if (this.config.floatEnable) {
        ...
        await this.floating.show(
          docs,
          bounding,
          Object.assign({}, this.config.floatConfig, {
  ...
  private getCompleteItem(
    item: VimCompleteItem | {} | null
  ): ExtendedCompleteItem | null {
