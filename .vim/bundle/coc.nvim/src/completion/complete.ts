...
export default class Complete {
    ...
    try {
      ...
      await new Promise<CompleteResult>((resolve, reject) => {
        ...
        let timer = setTimeout(() => {
          this.nvim.command(
            `echohl WarningMsg| echom 'source ${source.name} timeout after ${timeout}ms'|echohl None`,
            true
          );
          tokenSource.cancel();
        }, timeout);
        ...
        Promise.resolve(source.doComplete(opt, tokenSource.token)).then(
          (result) => {
            this.completing.delete(name);
            if (cancelled) return;
            onFinished();
      ...
      this.nvim.command(
        `echoerr 'Complete ${source.name} error: ${err.message.replace(
          /'/g,
          "''"
        )}'`,
        true
  ...
  public async completeInComplete(
    resumeInput: string
  ): Promise<ExtendedCompleteItem[]> {
  ...
  public filterResults(input: string, cid = 0): ExtendedCompleteItem[] {
    ...
    for (let i = 0, l = results.length; i < l; i++) {
      ...
      for (let idx = 0; idx < items.length; idx++) {
        ...
        let score =
          item.kind && filterText == input ? 64 : matchScore(filterText, codes);
        ...
        item.localBonus = this.localBonus
          ? this.localBonus.get(filterText) || 0
          : 0;
        words.add(word);
  ...
  private limitCompleteItems(
    ...
    return items.filter((item) => {
      ...
      if (
        (lowPrioritySourceLimit && isLow && curr == lowPrioritySourceLimit) ||
  ...
  public hasMatch(input: string): boolean {
    ...
    for (let i = 0, l = results.length; i < l; i++) {
      ...
      let idx = items.findIndex((item) =>
        fuzzyMatch(codes, item.filterText || item.word)
      );
  ...
  public async doComplete(): Promise<ExtendedCompleteItem[]> {
    ...
    if (this.config.localityBonus) {
      let line = linenr - 1;
      this.localBonus = this.document.getLocalifyBonus(
        Position.create(line, opts.col - 1),
        Position.create(line, colnr)
      );
    } else {
    ...
    let { results } = this;
    if (results.length == 0) return [];
    let engrossResult = results.find(
      (r) => r.startcol != null && r.startcol != col
    );
    if (engrossResult) {
  ...
  public resolveCompletionItem(
    ...
    try {
      ...
      for (let result of results) {
        let res = result.items.find(
          (o) => o.abbr == item.abbr && o.info == item.info
        );
        if (res) return res;
      }
      return null;
