...
export class DiagnosticBuffer implements BufferSyncItem {
  ...
  private refreshAle(
    ...
    let method = global.hasOwnProperty("__TEST__")
      ? "MockAleResults"
  ...
  public async refresh(
    diagnosticsMap: { [collection: string]: Diagnostic[] },
    force?: boolean
  ): Promise<void> {
  ...
  private async _refresh(
    ...
    let info = (await nvim.call("coc#util#diagnostic_info", [
      this.bufnr,
      checkInsert,
    ])) as DiagnosticInfo | undefined;
  ...
  public updateLocationList(winid: number, title: string): void {
    if (
      !this.config.locationlistUpdate ||
      winid == -1 ||
      title !== "Diagnostics of coc"
    )
      return;
    ...
    this.nvim.call(
      "setloclist",
      [winid, [], "r", { title: "Diagnostics of coc", items }],
      true
    );
  }
  ...
  public showVirtualText(lnum: number, bufnr?: number): void {
    ...
    let { virtualTextSrcId, virtualTextPrefix, virtualTextCurrentLineOnly } =
      this.config;
    ...
    for (let i = diagnostics.length - 1; i >= 0; i--) {
      ...
      void buffer.setVirtualText(
        virtualTextSrcId,
        line,
        [[virtualTextPrefix + msg, highlight]],
        {}
      );
    }
  }
  ...
  public getDiagnosticsAt(
    ...
    for (let diags of this.diagnosticsMap.values()) {
      if (checkCurrentLine) {
        diagnostics.push(
          ...diags.filter((o) => lineInRange(pos.line, o.range))
        );
      } else {
        diagnostics.push(
          ...diags.filter((o) => positionInRange(pos, o.range) == 0)
        );
      }
    }
    diagnostics.sort(sortDiagnostics);
