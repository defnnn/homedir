...
export class DiagnosticManager implements Disposable {
  ...
  public readonly onDidRefresh: Event<DiagnosticEventParams> =
  ...
  public async setLocationlist(bufnr: number): Promise<void> {
    ...
    let action =
      curr.title && curr.title.indexOf("Diagnostics of coc") != -1 ? "r" : " ";
    await this.nvim.call("setloclist", [
      0,
      [],
      action,
      { title: "Diagnostics of coc", items },
    ]);
  }
  ...
  public getDiagnostics(
    ...
    let collections = collection
      ? [this.getCollectionByName(collection)]
      : this.getCollections(uri);
  ...
  public async getCurrentDiagnostics(): Promise<Diagnostic[]> {
    let [bufnr, cursor, eol, lastline] = (await this.nvim.eval(
      `[bufnr("%"),coc#cursor#position(),col('.')==col('$')-1,line('.')==line('$')]`
  ...
  public async echoMessage(truncate = false): Promise<void> {
    ...
    // echo
    let [filetype, mode] = (await this.nvim.eval(`[&filetype,mode()]`)) as [
      string,
      string
    ];
    if (mode != "n") return;
    ...
    if (Object.keys(config.filetypeMap).length > 0) {
      ...
      ft =
        ...
        (defaultFiletype == "bufferType" ? filetype : defaultFiletype);
    }
    ...
    if (useFloat) {
      let config = this.floatFactory.applyFloatConfig(
        { modes: ["n"], maxWidth: 80 },
  ...
  private setConfiguration(event?: ConfigurationChangeEvent): void {
    ...
    if (
      messageTarget == "float" &&
      !workspace.env.floating &&
      !workspace.env.textprop
    ) {
      messageTarget = "echo";
    }
    let enableHighlightLineNumber = config.get<boolean>(
    ...
    this.config = {
      ...
      virtualText:
        config.get<boolean>("virtualText", false) &&
        this.nvim.hasFunction("nvim_buf_set_virtual_text"),
      virtualTextCurrentLineOnly: config.get<boolean>(
        "virtualTextCurrentLineOnly",
        true
      ),
      virtualTextPrefix: config.get<string>("virtualTextPrefix", " "),
      virtualTextLineSeparator: config.get<string>(
  ...
  public async toggleDiagnosticBuffer(bufnr: number): Promise<void> {
    ...
    if (buf) {
      ...
      await this.nvim.call("setbufvar", [
        bufnr,
        "coc_diagnostic_disable",
        isEnabled ? 1 : 0,
      ]);
      if (isEnabled) {
        buf.clear();
