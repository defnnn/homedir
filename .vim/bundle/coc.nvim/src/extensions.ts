...
export class Extensions {
  ...
  public ready = true;
  public readonly onDidLoadExtension: Event<Extension<API>> =
    this._onDidLoadExtension.event;
  public readonly onDidActiveExtension: Event<Extension<API>> =
    this._onDidActiveExtension.event;
  public readonly onDidUnloadExtension: Event<string> =
    this._onDidUnloadExtension.event;

  constructor() {
    let folder = global.hasOwnProperty("__TEST__")
      ? path.join(__dirname, "__tests__")
      : process.env.COC_DATA_HOME;
  ...
  public async init(): Promise<void> {
    ...
    stats.map((stat) => {
      let extensionType = stat.isLocal
        ? ExtensionType.Local
    ...
    commandManager.register(
      ...
      false,
      "remove all global extensions and install them"
    );
  ...
  public activateExtensions(): void {
    ...
    if (interval != "never") {
      ...
      let day = new Date(
        now.getFullYear(),
        now.getMonth(),
        now.getDate() - (interval == "daily" ? 0 : 7)
      );
  ...
  public async updateExtensions(
    ...
    let stats = await this.globalExtensionStats();
    stats = stats.filter(
      (o) => ![...lockedList, ...this.disabled].includes(o.id)
    );
    this.db.push("lastUpdate", Date.now());
    if (silent) {
      window.showMessage(
        "Updating extensions, checkout output:///extensions for details.",
        "more"
      );
    }
    let installBuffer = (this.installBuffer = new InstallBuffer(
      true,
      sync,
      silent ? this.outputChannel : undefined
    ));
    ...
    let fn = (stat: ExtensionInfo): Promise<void> => {
      ...
      return installer.update(url).then(
        (directory) => {
  ...
  public async getExtensionStates(): Promise<ExtensionInfo[]> {
    ...
    return localStats.concat(
      globalStats.filter((o) => localStats.find((s) => s.id == o.id) == null)
    );
  }
  ...
  public async reloadExtension(id: string): Promise<void> {
    ...
    if (item.type == ExtensionType.Internal) {
      window.showMessage(
        `Can't reload internal extension "${item.id}"`,
        "warning"
      );
      return;
    }
  ...
  public async uninstallExtension(ids: string[]): Promise<void> {
    try {
      ...
      let [globals, filtered] = splitArray(ids, (id) =>
        this.globalExtensions.includes(id)
      );
      if (filtered.length) {
        window.showMessage(
          `Extensions ${filtered} not global extensions, can't uninstall!`,
          "warning"
        );
      }
      let json = this.loadJson() || { dependencies: {} };
      ...
      fs.writeFileSync(jsonFile, JSON.stringify(sortedObj, null, 2), {
        encoding: "utf8",
      });
  ...
  public async loadExtension(folder: string): Promise<boolean> {
    try {
      ...
      let isLocal =
        path.normalize(parentFolder) != path.normalize(this.modulesFolder);
      ...
      this.createExtension(
        ...
        isLocal ? ExtensionType.Local : ExtensionType.Global
      );
      return true;
    } catch (e) {
      window.showMessage(
        `Error on load extension from "${folder}": ${e.message}`,
        "error"
      );
  ...
  public async watchExtension(id: string): Promise<void> {
    ...
    if (id.startsWith("single-")) {
      ...
      if (!client) {
        window.showMessage(
          `Can't create watchman client, check output:///watchman`
        );
        return;
      }
  ...
  private async globalExtensionStats(): Promise<ExtensionInfo[]> {
    ...
    let res: ExtensionInfo[] = await Promise.all(
      Object.keys(json.dependencies).map(
        (key) =>
          new Promise<ExtensionInfo>(async (resolve) => {
            try {
              let val = json.dependencies[key];
              ...
              if (res instanceof Error) {
                window.showMessage(
                  `Unable to load global extension at ${root}: ${res.message}`,
                  "error"
                );
  ...
  private async localExtensionStats(
    excludes: string[]
          ...
          new Promise<ExtensionInfo>(async (resolve) => {
            try {
              ...
              if (exist && !exist.isLocal) {
                logger.info(
                  `Extension "${obj.name}" in runtimepath already loaded.`
                );
                return resolve(null);
              }
              if (excludes.includes(obj.name)) {
                logger.info(
                  `Skipped load vim plugin from "${root}", "${obj.name}" already global extension.`
                );
                return resolve(null);
              }
              let version = obj ? obj.version || "" : "";
  ...
  private async setupActiveEvents(id: string, packageJSON: any): Promise<void> {
    ...
    let active = (): Promise<void> => {
      ...
      return new Promise((resolve) => {
        if (!this.canActivate(id)) {
          this.outputChannel.appendLine(
            `Extension ${id} is disabled or not loaded.`
          );
          return resolve();
        }
        let timer = setTimeout(() => {
          this.outputChannel.appendLine(
            `Extension ${id} activate cost more than 1s`
          );
          resolve();
        }, 1000);
        this.activate(id).then(
          ...
          (e) => {
            clearTimeout(timer);
            window.showMessage(
              `Error on activate extension ${id}: ${e.message}`
            );
            this.outputChannel.appendLine(
              `Error on activate extension ${id}:${e.message}\n ${e.stack}`
            );
    ...
    for (let eventName of activationEvents as string[]) {
      ...
      if (ev == "onLanguage") {
        ...
        let check = async () => {
          let folders = workspace.workspaceFolders.map(
            (o) => URI.parse(o.uri).fsPath
          );
        ...
        result = new Promise((resolve, reject) => {
          try {
            Promise.resolve(ext.activate(context)).then(
              ...
              (e) => {
                logger.error(
                  `Error on active extension ${id}: ${e.message}`,
                  e
                );
                reject(e);
              }
  ...
  public filterGlobalExtensions(names: string[]): string[] {
    ...
    for (let name of map.keys()) {
      ...
      if (
        (/^https?:/.test(name) && urls.some((url) => url.startsWith(name))) ||
  ...
  private get modulesFolder(): string {
    return path.join(
      this.root,
      global.hasOwnProperty("__TEST__") ? "" : "node_modules"
    );
  }
  ...
  private checkDirectory(folder: string): boolean | Error {
    try {
      ...
      if (main && !fs.existsSync(path.join(folder, main))) {
        throw new Error(
          `main file ${main} not found, you may need to build the project.`
        );
      }
      ...
      if (keys.includes("coc")) {
        ...
        if (!semver.satisfies(workspace.version, required)) {
          throw new Error(
            `Please update coc.nvim, ${packageJSON.name} requires coc.nvim ${engines["coc"]}`
          );
        }
      }
      return true;
